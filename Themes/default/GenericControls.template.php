...
function template_control_richedit($editor_id, $smileyContainer = null, $bbcContainer = null)
{
	...
	if(!isset($_REQUEST['preview']) && endsWith($editor_context['value'], '[/quote]'))
		$editor_context['value'] = $editor_context['value'] . ' ';
		...
		<script>
			...
			$(document).ready(function() {
				...
				$("#', $editor_id, '").sceditor({
					...
					style: "', $settings['theme_url'], '/css/jquery.sceditor.default', !empty($context['LoadWebkit']) ? '.webkit' : '', '.css",
...
/**
 * a PHP replacement for Javascript "endswidth"
 */
function endsWith($value, $find)
{
	return substr($value, -strlen($find)) == $find;
}
...
function template_control_richedit_buttons($editor_id)
{
	...
	if (!empty($context['drafts_pm_save']) && !empty($context['drafts_autosave']))
		...
		<script src="', $settings['default_theme_url'], '/scripts/drafts.js"></script>
		<script>
...
{
	global $context, $modSettings, $language, $user_info, $txt;

	$verify_context = &$context['controls']['verification'][$verify_id];

	// we have reChaptcha enabled ?
	if(isset($modSettings['recaptcha_enabled']) && !empty($modSettings['recaptcha_enabled']))
		$verify_context['can_recaptcha'] = intval(!empty($modSettings['need_capcha']));
	...
	$total_items = 0;
	if(isset($verify_context['questions']))
		$total_items = count($verify_context['questions']);
	if(isset($verify_context['show_visual']) || isset($verify_context['can_recaptcha']))
		$total_items += 1;
	...
	for ($i = 0; $i < $total_items; $i++)
	{
		...
		{
			if ($verify_context['show_visual'] && empty($verify_context['can_recaptcha']))
			{
				...
				echo '
					<div class="smalltext" style="margin: 4px 0 8px 0;">
						<a href="', $verify_context['image_href'], ';sound" id="visual_verification_', $verify_id, '_sound" rel="nofollow">', $txt['visual_verification_sound'], '</a> / <span class="visual_verification_href" id="visual_verification_', $verify_id, '_refresh">', $txt['visual_verification_request_new'], '</span><br>', '<br>
			...
			if (!empty($verify_context['can_recaptcha']))
				echo '
				<div class="g-recaptcha centertext" data-sitekey="'. $modSettings['recaptcha_private_key'] .'" data-theme="'. $modSettings['recaptcha_theme'] .'"></div>';
		}
		else
		{
