...
function template_main()
{
	...
	{
		echo '
	<div id="board_', $context['current_board'], '_childboards" class="boardindex_table childboards">
		<div class="cat_bar">
		...
		foreach ($context['boards'] as $board)
		{
			...
			if (!empty($board['last_post']['id']))
			{
				...
				{
					echo '_ava">
						<span class="avaspan"><img src="'. $board['last_post']['member']['avatar']['url'] .'" alt="avatar" class="';

					if(preg_match('/([a-z]|default)\.png/', $board['last_post']['member']['avatar']['url'], $test) > 0 && (is_array($test) && (count($test) == 2 || $test[1] == 'default')))
						echo 'brdidxava default"></span>';
					else
						echo 'brdidxava"></span>';
				}
				else
	...
	echo '
		...
		if (!empty($context['topics']))
		{
			echo '
					...
					<div class="lastpost_msgidx'. (!empty($modSettings['avatars_on_boardIndex']) ? '_ava' : '') .'" style="padding-top: 9px;">', $context['topics_headers']['last_post'], '</div>';
		...
		foreach ($context['topics'] as $topic)
		{
			...
			{
				if($topic['new_from'] == 1)
					$topic['new_href'] = $topic['last_post']['href'];

				echo '
								<a href="'. $topic['new_href'] .'" id="newicon'. $topic['last_post']['id'] .'"><span class="new_posts">'. $txt['new'] .'</span></a>';
			}
			echo '
					...
					<div class="board_stats centertext"><p>', $txt['replies'], ': ', $topic['replies'], '<br>', $txt['views'], ': ', $topic['views'], '</p></div>
					<div class="lastpost_msgidx';
...
{
	global $context, $settings, $txt, $user_info, $modSettings;
	...
	if (empty($context['no_topic_listing']))
		echo '
			...
			<p>
				...
				<span class="generic_icons sticky centericon"></span> ' . $txt['sticky_topic'] .'<br>'. ($user_info['is_admin'] ? '
				<img src="' . $settings['images_url'] . '/post/recycled.png" alt="" class="centericon sizefix"> ' . $txt['recycled_topic'] . '<br>' : '') .'
			</p>';

	if (!empty($context['jump_to']) && isset($context['jump_to']['child_level']) && $context['jump_to']['child_level'] > 1)
		echo '
			<script>
