...
<form class="flow_hidden" action="', $scripturl, '?action=pm;sa=pmactions;', $context['display_mode'] == 2 ? 'conversation;' : '', 'f=', $context['folder'], ';start=', $context['start'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', '" method="post" accept-charset="', $context['character_set'], '" name="pmFolder">';
	...
	if ($context['get_pmessage']('message', true))
	{
		...
		if ($context['display_mode'] != 1)
			echo '
				<div class="cat_bar botrnd'. ($context['display_mode'] != 2 ? '_pad' : '') .'">
					<h3 class="catbg">
						<span class="poster" style="border-bottom:0;padding-left:0;margin-right:8px;" id="author">', $txt['author'], '</span>
						<span>', $txt[$context['display_mode'] == 0 ? 'messages' : 'conversation'], '</span>
					</h3>
				</div>';
		...
		if ($context['display_mode'] == 2)
		{
			...
			echo '
					<div class="pagesection pm_section">';
		...
		{
			echo '
	<div class="windowbg" id="top" style="'. ($first == 0 ? ' margin-top:0px;' : 'margin-top:15px;') .'">
		<div class="post_wrapper">
			...
			if (!empty($modSettings['show_user_images']) && !empty($modSettings['avatars_on_boardIndex']))
			{
				if(!isset($message['member']['avatar']))
				{
					$message['member']['avatar']['class'] = 'default';
					$message['member']['avatar']['image'] = '<img class="avatar" src="'. $modSettings['avatar_url'] .'/default.png" alt="avatar" oncontextmenu="return false">';
				}
				echo '
				<li class="avatar'. (isset($message['member']['avatar']['class']) ? '_pm '. $message['member']['avatar']['class'] : '') .'">
			...
			if (!$message['member']['is_guest'])
			{
				...
				<li class="profile">
					...
					if(isset($message['member']['gender']) && !isset($context['disabled_fields']['gender'])) 
						echo '
		...
		if (empty($context['display_mode']))
			echo '
	<div class="pagesection pm_section bottom" style="height:auto;">
		<div class="pagelinks floatleft">', $context['page_index'], '</div>
		<div class="floatright" style="margin-top:5px;"><input type="submit" name="del_selected" value="', $txt['quickmod_delete_selected'], '" style="font-weight: normal;" onclick="if (!confirm(\'', $txt['delete_selected_confirm'], '\')) return false;" class="button_submit"></div>
	</div>';
...
function template_subject_list()
{
	...
	<table class="table_grid">
		<tr class="title_bar">
			...
			<th class="lefttext" width="22%" style="padding-left:8px;">
			...
			<th class="lefttext" width="46%" style="padding-left:8px;"
			...
			<th class="lefttext" style="padding-left:8px;">
...
function template_search()
{
	...
	{
		echo '
		<div class="errorbox" style="margin-bottom:0;">
		...
		<fieldset id="advanced_search">
			<div class="information" style="padding-top:15px;">
				...
				<dl id="search_options">
					<dt><strong>', $txt['pm_search_text'], ':</strong></dt>
					<dd>
	...
	{
			echo '
			<div class="cat_bar botrnd">
				<h3 class="catbg">
					<span id="advanced_panel_toggle" class="toggle_up floatright"></span><a href="#" id="advanced_panel_link">', $txt['pm_search_choose_label'], '</a>
				</h3>
			</div>
			<div class="information" id="advanced_panel_div" style="margin-bottom:0px;">
		...
		echo '
			...
			<p style="margin-top:10px;padding:0 16px;">
...
function template_send()
{
	...
	if (!empty($modSettings['drafts_pm_enabled']))
		echo '
				<div id="draft_section" class="infobox">',
	...
	if ($context['require_verification'] || !empty($modSettings['need_capcha']))
	{
		echo '
				<div class="post_verification windowbg2 centertext">
...
function template_labels()
{
	...
	if (!count($context['labels']) < 2)
		echo '
		<div class="padding" style="height:25px;">
