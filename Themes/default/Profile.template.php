...
function template_GDPR_ExportData()
{
}
...
function template_profile_popup()
{
	...
	if(empty(count($context['profile_items']) % 2))
		echo '<li></li>'; 

	echo '
				<li>
					<span class="generic_icons logout"></span><a href="', $scripturl, '?action=logout;'. $context['session_var'] .'='. $context['session_id'] .'"><strong>', $txt['logout'], '</strong></a>
				</li>';
	...
	/*
		Only if Portal enable..
		Look at all the Portal rights they a user can have
	*/
	if(!empty($modSettings['portal_enabled']))
	{
		PortaMx_getSettings(true);
		if(allowedTo('profile_view_own'))
		{
			if(allowPmx('pmx_admin, pmx_blocks, pmx_articles, pmx_create', true))
			{
				echo '
			<div style="margin:4px 0 8px 0 ;border-top: solid 1px #cacaca !important;"></div>
			<ol>';

				if(allowPmx('pmx_admin', true))
					echo '
				<li>
					<span class="generic_icons portal"></span><a href="'. $scripturl . '?action=portal;area=pmx_center;'. $context['session_var'] .'='. $context['session_id'] .'">'. $txt['pmx_ext_center'] .'</a>
				</li>';

				if(allowPmx('pmx_admin, pmx_settings', true))
					echo '
				<li>
					<span class="generic_icons settings"></span><a href="'. $scripturl . '?action=portal;area=pmx_settings;'. $context['session_var'] .'='. $context['session_id'] .'">'. $txt['pmx_settings'] .'</a>
				</li>';

				if(allowPmx('pmx_admin, pmx_blocks', true))
					echo '
				<li>
					<span class="generic_icons blocks"></span><a href="'. $scripturl . '?action=portal;area=pmx_blocks;'. $context['session_var'] .'='. $context['session_id'] .'">'. $txt['pmx_blocks'] .'</a>
				</li>';

				if(allowPmx('pmx_admin, pmx_articles, pmx_create', true))
					echo '
				<li>
					<span class="generic_icons articles"></span><a href="'. $scripturl . '?action=portal;area=pmx_articles;'. $context['session_var'] .'='. $context['session_id'] .'">'. $txt['pmx_articles'] .'</a>
				</li>';

				if(allowPmx('pmx_admin, pmx_categories', true))
					echo '
				<li>
					<span class="generic_icons categories"></span><a href="'. $scripturl . '?action=portal;area=pmx_categories;'. $context['session_var'] .'='. $context['session_id'] .'">'. $txt['pmx_categories'] .'</a>
				</li>';
...
{
	global $context, $txt, $options, $modSettings, $scripturl;
	...
	else
	{
		...
		{
			if(!empty($details['sender']))
				$details['sender']['avatar']['image'] = preg_replace('~class\=\"[^\"]*\"~', 'class="avatar '. $details['sender']['avatar']['class'] .'"', $details['sender']['avatar']['image']);

			echo '
			<div class="unread">
				', !empty($modSettings['avatars_on_boardIndex']) && !empty($details['sender']) ? '<span class="avaimg">'. $details['sender']['avatar']['image'] .'</span>' : '', '
				<div class="details">
...
function template_summary()
{
	...
	$context['member']['avatar']['image'] = preg_replace('~class\=\"[^\"]*\"~', 'class="avatar '. $context['member']['avatar']['class'] .'"', $context['member']['avatar']['image']);
	echo '
	...
	if ($context['member']['website']['url'] !== '' && !isset($context['disabled_fields']['website']))
		echo '
				<li><a href="', $context['member']['website']['url'], '" title="' . $context['member']['website']['title'] . '" target="_blank" rel="noopener" class="new_win">', ($settings['use_image_buttons'] ? '<span class="generic_icons www" title="' . $context['member']['website']['title'] . '"></span>' : $txt['www']), '</a></li>';

	// show gender.
	echo '
				<li class="gender'. $context['member']['gender'] .'"><span class="generic_icons gender_'. $context['member']['gender'] .'" title="'. $txt['cust_gender']['field_name'] .': '. $txt['gender_type'][$context['member']['gender']] .'"></span></li>';
	...
	// show location.
	echo '
			<br />'. $txt['cust_loca']['field_name'] .': ', !empty($context['member']['location']) ? $context['member']['location'] : $txt['no_location'];
	...
	if (!empty($context['member']['language']))
		echo '
					<dt>', $txt['language'], ':</dt>
		...
		<form action="', $scripturl, '?action=profile;u=', $context['id_member'], ';area=showalerts;save" method="post" accept-charset="', $context['character_set'], '" id="mark_all">
			<div class="div_overflow">
				<div  style="min-height:30px;">
					<div class="pagesection" id="ptop">
						<a href="#pbot" class="topbottom floatleft">'. $txt['go_down'] .'</a>
						<div class="pagelinks floatleft">'. $context['pagination'] .'</div>
					</div>
				</div>

			<table id="alerts" class="table_grid" style="margin-bottom:0;">';

		foreach ($context['alerts'] as $id => $alert)
		{
			if(isset($alert['sender']['avatar']['image']))
				$alert['sender']['avatar']['image'] = preg_replace('~class\=\"[^\"]*\"~', 'class="avatar '. $alert['sender']['avatar']['class'] .'"', $alert['sender']['avatar']['image']);

			echo '
				<tr class="windowbg">
					<td class="first_col">
						', !empty($modSettings['avatars_on_boardIndex']) && isset($alert['sender']['avatar']['image']) ? '<span class="avaimg">'. $alert['sender']['avatar']['image'] .'</span>' : '', ' 
						', isset($alert['text']) ? $alert['text'] : '???', '</td>
					<td>', $alert['time'], '</td>
					<td>
						<ul class="quickbuttons">
							...
							<li><input type="checkbox" class="input_check" name="mark[', $id, ']" value="', $id, '" style="height:14px;"></li>
						</ul>
					</td>
				</tr>';
		}
				...
				<tr>
					<td colspan="3" style="padding:8px 0 0 0;">
						<div style="text-align:right;white-space:nowrap;">
							', $txt['check_all'], ': <input type="checkbox" class="input_check" name="select_all" id="select_all" style="vertical-align:middle;">
							<select name="mark_as">
								...
								<option value="remove_all" style="font-weight:bold">', $txt['quick_mod_remove_all'] ,'</option>
							</select>
							<input type="hidden" name="', $context['session_var'], '" value="', $context['session_id'], '">
							<input type="submit" name="req" value="', $txt['quick_mod_go'] ,'" class="button_submit you_sure" style="text-align:right;float:none;">
						</div>
						<div class="pagesection" id="pbot" style="margin:0;">
							<a href="#ptop" class="topbottom floatleft">'. $txt['go_up'] .'</a>
							<div class="pagelinks floatleft">'. $context['pagination'] .'</div>
						</div>
					</td>
...
function template_showDrafts()
{
	...
	if(!empty($context['page_index']) && !empty($context['drafts']))
		echo '
	...
	if (empty($context['drafts']))
		echo '
		<div class="tborder windowbg2 totop centertext">
			'. $txt['draft_none'] .'
		</div>';
	else
	{
...
function template_editBuddies()
{
	...
	<div id="edit_buddies">
		...
		<table class="table_grid">
			<tr class="title_bar">
				<th scope="col" class="buddys">', $txt['name'], '</th>
...
function template_editIgnoreList()
{
	...
	<div id="edit_buddies">
		...
		<table class="table_grid">
			<tr class="title_bar">
				<th scope="col" width="20%" class="buddys">', $txt['name'], '</th>
				<th scope="col">', $txt['status'], '</th>';
	...
	foreach ($context['ignore_list'] as $member)
	{
		echo '
			<tr class="windowbg">
				<td style="text-align:left;">', $member['link'], '</td>
...
function template_trackActivity()
{
	...
	echo '
		<div id="tracking" class="windowbg totop modnotes">
			<dl class="noborder">
...
function template_trackIP()
{
		...
		<div class="windowbg2 totop">
			<form action="', $context['base_url'], '" method="post" accept-charset="', $context['character_set'], '">
				<dl class="settings">
					...
					<dd>
						<input type="text" name="searchip" value="', $context['ip'], '" class="input_text" style="width:100%;">
					</dd>
				</dl>
				<input type="submit" value="', $txt['trackIP'], '" class="button_submit" style="margin-right:5px;">
			</form>
		</div>
	</div>
	<br>';
	...
	if ($context['single_ip'])
	{
		echo '
			...
			echo '
				<a href="', $server['url'], '" target="_blank" rel="noopener" class="new_win"', isset($context['auto_whois_server']) && $context['auto_whois_server']['name'] == $server['name'] ? ' style="font-weight: bold;"' : '', '>', $server['name'], '</a><br>';
			echo '
			</div>
			<br>';
	}
	...
	if (empty($context['ips']))
		echo '
		<p class="windowbg2 description totop"><em>', $txt['no_members_from_ip'], '</em></p>';
	else
	{
		echo '
		<table class="table_grid">
...
function template_statPanel()
{
	...
	else
	{
		...
		{
			$tm = explode(' ', $time_of_day['hour_format']);
			echo '
				<li', $time_of_day['is_last'] ? ' class="last"' : '', '>
					...
					<span class="stats_hour">';
			if(count($tm) > 1)
			{
				echo $tm[0] .'<br>';
				if($i == 0)
					echo '<span class="activity_ae">&#9664;</span>';
				elseif($i == 5)
					echo '<span class="activity_cnt">'. $tm[1] .'</span>';
				elseif($i == 11)
				{
					echo '<span class="activity_ae">&#9654;</span>';
					$i = -1;
	...
	else
	{
		...
		foreach ($context['popular_boards'] as $board)
		{
			echo '
					<dt>', $board['link'], '</dt>
					<dd>
						<div class="profile_pie" style="background-position: -', strval(intval(intval($board['posts_percent']) / 5) * 20), 'px 0;" title="', sprintf($txt['statPanel_topBoards_memberposts'], $board['posts'], $board['total_posts_member'], $board['posts_percent']), '">
	...
	else
	{
		...
		foreach ($context['board_activity'] as $activity)
		{
			echo '
					<dt>', $activity['link'], '</dt>
					<dd>
						<div class="profile_pie" style="background-position: -', strval(intval(intval($activity['percent']) / 5) * 20), 'px 0;" title="', sprintf($txt['statPanel_topBoards_posts'], $activity['posts'], $activity['total_posts'], $activity['posts_percent']), '">
		...
		foreach ($context['custom_fields'] as $field)
		{
			echo '
					<dt>
						...
						<span class="smalltext">', $txt[$field['colname']], '</span>
					</dt>
					<dd>
						', $field['input_html'], '
					</dd>';
	...
	if(!empty($res))
		echo '
							...
							<dd>
								...
								<input type="checkbox" name="default_options[display_quick_mod]" id="use_display_quick_mod" value="1"', !empty($context['member']['options']['display_quick_mod']) ? ' checked' : '', ' class="input_check">
							</dd>';
}
...
function template_alert_configuration()
{
	...
	echo '
		...
		<form action="', $scripturl, '?', $context['action'], '" id="admin_form_wrapper" method="post" accept-charset="', $context['character_set'], '" id="notify_options" class="flow_hidden">
			...
			<div class="windowbg2" style="margin-bottom:10px;">
				<dl class="settings">';
...
function template_ignoreboards()
{
	...
	$i = 0;
	$limit = ceil($context['num_boards'] / 2) +1;
...
function template_issueWarning()
{
	...
	if (!$context['user']['is_owner'])
	{
		...
		echo '
					...
					<span class="smalltext" id="new_template_link" style="display: none;">[<a href="', $scripturl, '?action=moderate;area=warnings;sa=templateedit;tid=0" target="_blank" rel="noopener" class="new_win">', $txt['profile_warning_new_template'], '</a>]</span><br>
...
function template_profile_birthdate()
{
	...
	$str = '<span class="smalltext">';
	$inp =  '';
	for($i = 0; $i < 3; $i++)
	{
		$str .= $txt['dob_'. $txt['inputs_bd'][$i][1]] . ($i < 2 ? ' - ' : '');
		$inp .= '<input type="text" name="'. $txt['inputs_bd'][$i][0] .'" size="'. $txt['inputs_bd'][$i][2] .'" maxlength="'. $txt['inputs_bd'][$i][2] .'" value="'. $context['member']['birth_date'][$txt['inputs_bd'][$i][1]] .'" class="input_text">' .($i < 2 ? ' - ' : '');
	}
...
function template_profile_avatar_select()
{
	...
	if (!empty($context['member']['avatar']['allow_server_stored']))
	{
		...
		if(preg_match('~[a-zA-Z]+~', $context['member']['name'], $tmp) > 0)
		{
			$default_image = $modSettings['avatar_url'] .'/'. strtolower(substr($tmp[0],0,1)) .'.png';
			$class = '"avatar default"';
		}
		echo '
								<div id="default_avatar"><img src="'. $default_image .'" alt="'. $context['member']['name'] .'" class='. $class .' oncontextmenu="return"'. ($context['member']['avatar']['choice'] == 'none' ? ' style="display:block"' : '') .' /></div>';
	...
	if (!empty($context['member']['avatar']['allow_gravatar']))
	{
		...
		if(preg_match('/[a-z\?\.]png/', $context['member']['avatar']['href'], $tmp) == 0)
			echo '
	...
	if($context['member']['avatar']['choice'] == 'server_stored')
		$ava = 'avatar_server_stored';
	else
		$ava = 'default_avatar';
	...
	echo '
								<script>
									...
									', !empty($context['member']['avatar']['allow_server_stored']) ? 'document.getElementById("'. $ava .'").style.display = "' . ($context['member']['avatar']['choice'] == 'server_stored' || $context['member']['avatar']['choice'] == 'none' ? '' : 'none') . '";' : '', '
									...
									function swap_avatar(type)
									{
										switch(type.id)
										{
											...
											case "avatar_choice_none":
												document.getElementById("default_avatar").style.display = "";', '
												', !empty($context['member']['avatar']['allow_server_stored']) ? 'document.getElementById("avatar_server_stored").style.display = "none";' : '', '
												', !empty($context['member']['avatar']['allow_external']) ? 'document.getElementById("avatar_external").style.display = "none";' : '', '
												', !empty($context['member']['avatar']['allow_upload']) ? 'document.getElementById("avatar_upload").style.display = "none";' : '', '
												', !empty($context['member']['avatar']['allow_gravatar']) ? 'document.getElementById("avatar_gravatar").style.display = "none";' : '', '
