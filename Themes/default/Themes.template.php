...
function template_list_themes()
{
	...
	{
		echo '
			<div class="cat_bar'. ($cnt > 0 ? ' addtopspace' : '') .'">
				<h3 class="catbg">
			...
			if ($theme['id'] != 1)
			{
				...
				// Enable/Disable.
				echo '
						<a href="', $scripturl, '?action=admin;area=theme;sa=enable;th=', $theme['id'], ';', $context['session_var'], '=', $context['session_id'], ';', $context['admin-tre_token_var'], '=', $context['admin-tre_token'], '', (!empty($theme['enable']) ? ';disabled' : '') ,'" data-confirm="', $txt['theme_'. (!empty($theme['enable']) ? 'disable' : 'enable') .'_confirm'], '" class="you_sure"><span class="smalltext">[', $txt['theme_'. (!empty($theme['enable']) ? 'disable' : 'enable')] ,']</span></a>&nbsp;';

				// Deleting.
				echo '
						<a href="', $scripturl, '?action=admin;area=theme;sa=remove;th=', $theme['id'], ';', $context['session_var'], '=', $context['session_id'], ';', $context['admin-tr_token_var'], '=', $context['admin-tr_token'], '" data-confirm="', $txt['theme_remove_confirm'], '" class="you_sure"><span class="smalltext">[', $txt['theme_remove'], ']</span></a>';
			...
			<div class="windowbg2'. ($cnt == 0 ? ' addbotspace' : '') .'">
...
function template_pick()
{
	...
	foreach ($context['available_themes'] as $theme)
	{
		echo '
			...
			<div class="', $theme['selected'] ? 'windowbg' : 'windowbg2', ' totop_pad">
...
function template_edit_browse()
{
	...
	foreach ($context['theme_files'] as $file)
	{
		...
		echo '
				...
				<td class="righttext quarter_table">', !empty($file['last_modified']) ? $file['last_modified'] : '', '</td>
				<td class="righttext quarter_table">', $file['size'], '</td>
			</tr>';
	}
