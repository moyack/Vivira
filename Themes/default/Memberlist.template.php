...
function template_main()
{
	...
	foreach ($context['columns'] as $key => $column)
	{
		...
		if($key == 'location')
			$column['link'] = str_replace('>'. $column['label'] .'<', '>'. $txt['cust_loca']['field_name'] .'<', $column['link']);
	...
	if (!empty($context['members']))
	{
		...
		if (!isset($context['disabled_fields']['website']))
			echo '
					<td class="centertext website_url">', $member['website']['url'] != '' ? '<a href="' . $member['website']['url'] . '" target="_blank" rel="noopener" class="new_win"><span class="generic_icons www" title="' . $member['website']['title'] . '"></span></a>' : '', '</td>';

		// Group and date.
		echo '
					<td class="centertext reg_group">', empty($member['group']) ? $member['post_group'] : $member['group'], '</td>
					<td class="centertext reg_date">', date_format(date_create_from_format($txt['dp_from_format'], $member['registered_date']), $txt['dp_to_format']), '</td>';

		if (!isset($context['disabled_fields']['posts']))
		{
			echo '
					<td class="centertext" style="text-align:right;">', $member['posts'], '</td>
					<td class="centertext statsbar">';

			if (!empty($member['post_percent']))
				echo '
						<div class="bar" style="width: ', $member['post_percent'], '%;">
		...
		if (!isset($context['disabled_fields']['location']))
			echo '
					<td class="location righttext">', $member['location'], '</td>';
...
function template_search()
{
	...
	{
		if (!isset($context['disabled_fields'][$id]))
			echo '
						<dd>
