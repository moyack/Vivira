...
{
	global $context, $options, $txt, $modSettings, $scripturl;
		...
		echo '
			...
			foreach ($category['boards'] as $board)
			{
				if(!$board['is_redirect'])
					$board['href'] = str_replace('#new', '', $board['href']) .'#ptop';
				...
				if($board['is_redirect'])
					echo '<p class="redir">', $txt['redirects'] ,':<br />', $board['posts'] ,'</p>';
				else
					echo '<p>', $txt['posts'] ,': ', comma_format($board['posts']), '<br />', $txt['board_topics'] ,': ', comma_format($board['topics']), '</p>';

				echo '
					</div>
					<div class="lastpost'. (!empty($modSettings['avatars_on_boardIndex']) ? '_ava' : '') .' lpr_border">';

				if (!empty($board['last_post']['id']))
				{
					if(isset($board['last_post']['member']['avatar']['url']) && !empty($modSettings['avatars_on_boardIndex']))
					{
						echo '
						<span class="avaspan"><img src="'. $board['last_post']['member']['avatar']['url'] .'" alt="ava" class="';
						if(preg_match('/([a-z]|default)\.png/', $board['last_post']['member']['avatar']['url'], $test) > 0 && (is_array($test) && (count($test) == 2 || $test[1] == 'default')))
							echo 'brdidxava default';
						else
							echo 'brdidxava';
						echo '"></span>';
					}
				...
				if (!empty($board['children']))
				{
					...
					foreach ($board['children'] as $child)
					{
						...
						$children[] = $child['new'] ? '<strong>' . trim($child['link']) . '</strong>' : trim($child['link']);
					}
