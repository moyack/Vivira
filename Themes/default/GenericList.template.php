...
function template_show_list($list_id = null)
{
	...
	$isModLog = getREQcnt('moderate,modlog') == 2;

	// Show the title of the table (if any).
	if (!empty($cur_list['title']) && empty($isModLog))
		echo '
	...
	if (isset($cur_list['additional_rows']['after_title']) && empty($isModLog))
	{
		echo '
	...
	if ((!empty($cur_list['items_per_page']) && !empty($cur_list['page_index'])) || isset($cur_list['additional_rows']['above_column_headers']))
	{
		...
		if (!empty($cur_list['items_per_page']) && !empty($cur_list['page_index']))
			echo '
			<div class="floatleft"'. (getREQcnt('admin,manageattachments,browse') > 1 ? ' style="margin-top:5px;"' : (getREQcnt('admin,tasklog') > 1 ? ' style="margin-top:-6px;"' : '')) .'>
				<div class="fixpagesbar" id="top"'. (getREQcnt('holidays,managecalendar') > 0 ? ' style="margin-top:2px;"' : (getREQcnt('sitemap') > 0 ? ' style="margin-top:6px;"' : '')) .'>
	...
	{
		if($_REQUEST['area'] == 'smileys' && isset($_REQUEST['sa']) && $_REQUEST['sa'] != 'editicons')
			$useDiv = '';
//		elseif($_REQUEST['area'] == 'manageattachments')
//			$useDiv = '';
	}
	echo '
			<div class="clear'. $useDiv .'">
	...
	{
		echo '
			<div'. (getREQcnt('tracking,ip,edits') == 2 ? ' style="height:25px;"' : (getREQcnt('sengines') == 1 && getREQcnt('settings,spiders,logs') == 0 ? ' style="min-height:40px;"' : '')) .'>';
		...
		if (!empty($cur_list['items_per_page']) && !empty($cur_list['page_index']))
			echo '
				<div class="floatleft">
					<div class="pagesbar" id="pbot"'. (isset($_REQUEST['area']) && $_REQUEST['area'] == 'modlog' ? ' style="margin-top:'. (empty($scrMode[1]) || intval($scrMode[1]) > 400 ? '-25px;' : '0px') .'"' : '') .'>
...
function template_additional_rows($row_position, $cur_list)
{
	...
	{
		if(getREQcnt('admin,manageattachments,attachpaths') == 3)
			$row['class'] = (isset($row['class']) ? $row['class'] .' addbotspace' : 'addbotspace');
		echo '
			<div class="additional_row '. (empty($row['class']) ? '' : $row['class']) .' '. $row_position .'"', empty($row['style']) ? '' : ' style="' . $row['style'] . '"', '>', $row['value'], '</div>';
	}
}
