...
function template_accept_gdpr()
{
	global $context, $scripturl, $modSettings, $txt;

	echo '
		<form action="', $scripturl, '" method="post" accept-charset="', $context['character_set'], '">
			<div class="cat_bar"', (getREQcnt('mlist') > 0 ? ' style="margin-top:5px;"' : ''), '>
				<h3 class="catbg"  style="text-align:center">', (!empty($modSettings['gdpr_enabled']) ? $txt['gdpr_registration_agreement'] : $txt['registration_agreement']), '</h3>
			</div>
			<div class="information">
				<p>', $context['agreement'], '</p>
			</div>
			<div id="confirm_buttons">
				<input type="submit" name="agree_gdpr" value="', (!empty($modSettings['gdpr_enabled']) ? $txt['gdpr_agreement_agree'] : $txt['agreement_agree']), '" class="button_submit">
...
function template_registration_agreement()
{
	...
	echo '
		<form action="', $scripturl, '?action=signup" method="post" accept-charset="', $context['character_set'], '" id="registration">
			<div class="cat_bar">
				<h3 class="catbg" style="text-align:center">', $txt['registration_agreement'], '</h3>
			</div>
			<div class="information">
				<p>', $context['agreement'], '</p>
			</div>
...
function template_registration_form()
{
	...
	echo '
		<form action="', !empty($modSettings['force_ssl']) && $modSettings['force_ssl'] < 2 ? strtr($scripturl, array('http://' => 'https://')) : $scripturl, '?action=signup2" method="post" accept-charset="', $context['character_set'], '" name="registration" id="registration" onsubmit="return verifyAgree();">
			...
			<div class="title_bar" style="margin-top:1px;">
	...
	if ($context['visual_verification'] || !empty($modSettings['need_capcha']))
	{
		echo '
			<div class="title_bar title_top">
...
function template_coppa()
{
	...
	echo '
			...
			<div id="coppa" class="roundframe">
				...
				<p>
					<span><a href="', $scripturl, '?action=coppa;form;member=', $context['coppa']['id'], '" target="_blank" rel="noopener" class="new_win">', $txt['coppa_form_link_popup'], '</a> | <a href="', $scripturl, '?action=coppa;form;dl;member=', $context['coppa']['id'], '">', $txt['coppa_form_link_download'], '</a></span>
				</p>
		...
		<form id="admin_form_wrapper" action="', $scripturl, '?action=admin;area=regcenter" method="post" accept-charset="', $context['character_set'], '">
			...
			<div class="windowbg2 noup">
				...
				<dl class="settings">
					...
					<dt>
						<label for="matchname">', $txt['admin_check_display_logging'], '</label>
					</dt>
					<dd>
						<input type="checkbox" name="write_tolog" id="write_tolog" tabindex="', $context['tabindex']++, '"', $context['reserved_word_options']['write_tolog'] ? ' checked' : '', ' class="input_check">
					</dd>
				</dl>
				<div class="flow_auto">
