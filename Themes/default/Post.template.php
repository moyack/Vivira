...
function template_main()
{
	...
	echo '
						...
						<dd>
							<input type="text" name="subject"', $context['subject'] == '' ? '' : ' value="' . $context['subject'] . '"', ' tabindex="', $context['tabindex']++, '" size="65" maxlength="65"', isset($context['post_error']['no_subject']) ? ' class="error"' : ' class="input_text"', ' required>
						</dd>
						<dt class="clear_left">
							<div style="height:24px;">', $txt['message_icon'], '</div>
						</dt>
						<dd>';

	if ($context['browser']['is_webkit'] !== false)
		echo '
							<select name="icon" id="icon" style="position:absolute;z-index:5;">';
	else
		echo '
							<select name="icon" id="icon" onchange="showimage()" style="position:absolute;z-index:5;">';
	...
	echo '
							...
							<div style="height:30px;"><img src="', $context['icon_url'], '" id="icons" alt=""></div>';

	if ($context['browser']['is_webkit'] !== false)
		echo '
							<script>
								var ddelm = document.getElementById("icon");
								var OldIdx = ddelm.selectedIndex;
								var OrgWidth = parseInt($(ddelm).css("width"));
								var IconElm = document.getElementById("icons");
								var optH = parseInt($("option").css("height"));
								var closedH = parseInt($("select").css("height"));
								var aniheight = (12 * optH) + 6;
								function IconSetup()
								{
									IconElm.focus();
									if(!mobile_device)
									{
										ddelm.size = 12;
										$(ddelm).animate({height: aniheight},{duration: 50, easing: "linear", complete: function(){ddelm.scrollTop = OldIdx * optH;}});
									}
									ddelm.onmousedown = function(){};
									ddelm.onblur = function(){IconReset();}
									ddelm.onclick = function(){IconSelected()};
									ddelm.style.width = OrgWidth +"px";
									firstClick = true;
									showimage();
									return false;
								}
								function IconSelected()
								{
									...
									{
										if(!mobile_device)
											ddelm.scrollTop = ddelm.selectedIndex * optH;
										OldIdx = ddelm.selectedIndex;
										IconReset();
									}
								}
								function IconReset()
								{
									...
									if(!mobile_device)
										$(ddelm).animate({height: closedH},{duration: 50, easing: "linear", complete: function(){ddelm.size = 1;}});
									ddelm.style.width = OrgWidth + "px";
									showimage();
									ddelm.onmousedown = function(){IconSetup()};
								}
								IconReset();
							</script>';
	...
	if ($context['make_event'])
	{
		...
		for ($month = 1; $month <= 12; $month++)
			echo '
									<option value="', $month, '"', $month == $context['event']['month'] ? ' selected' : '', '>', $txt['months_titles'][$month], '&nbsp;</option>';
	...
	if ($context['require_verification'] || !empty($modSettings['need_capcha']))
	{
		echo '
					<div class="post_verification windowbg2 centertext">
						<span', (!empty($context['post_error']['need_qr_verification']) || !empty($context['post_error']['recapcha_post_preview']) ? ' class="error"' : ''), '>
					...
					for (var i = 0, n = textFields.length; i < n; i++)
						if (textFields[i] in document.forms.postmodify)
						{
							...
							if (textFields[i] == ', JavaScriptEscape($context['post_box_name']), ' && $("#', $context['post_box_name'], '").data("sceditor") != undefined)
								x[x.length] = textFields[i] + \'=\' + $("#', $context['post_box_name'], '").data("sceditor").getText().html_entity_decode.php_urlencode();
							else
	...
	if (isset($context['previous_posts']) && count($context['previous_posts']) > 0)
	{
		...
		foreach ($context['previous_posts'] as $post)
		{
			...
			{
				echo '
					<ul class="post_quote quickbuttons" id="msg_', $post['id'], '_quote">
