		...
		echo '
			...
			<div class="windowbg2">
				...
				<span class="floatright">', $txt['package_available_readme_language'], '
					<select name="readme_language" id="readme_language" onchange="setReadmeLang(this);">';
			...
			<script>function setReadmeLang(selelm){lang=selelm.options[selelm.selectedIndex].value;window.location.href = pmx_prepareScriptUrl(pmx_scripturl) + \'', 'action=admin;area=packages;sa=', $context['uninstalling'] ? 'uninstall' : 'install', ';package=', $context['filename'], ';readme=\' + lang;}</script>
			<br>';
	}
		...
		echo '
			...
			<div class="windowbg2">
				...
				<span class="floatright">', $txt['package_available_license_language'], '
					<select name="license_language" id="license_language" onchange="if (this.options[this.selectedIndex].value) window.location.href = pmx_prepareScriptUrl(pmx_scripturl + \'', '?action=admin;area=packages;sa=install', ';package=', $context['filename'], ';license=\' + this.options[this.selectedIndex].value + \');">';
...
function template_servers()
{
	...
	echo '
			<div class="windowbg2">
				<fieldset>
					<legend style="padding:0 8px 0 5px;">' . $txt['package_servers'] . '</legend>
	...
	foreach ($context['servers'] as $server)
	{
		echo '
						<li class="flow_auto">
							<span class="floatleft" style="padding-left:5px;'. ($server['id'] === '1' ? 'font-weight:bold;font-style:italic;' : '') .'">'.  $server['name'] .'</span>';
	...
	echo '
				...
				<fieldset>
					<legend>
						<a class="download_new_package">
							<span class="toggle_down floatleft" title="', $txt['show'], '" style="margin-left:-2px;"></span>
						</a>
						' . $txt['add_server'] . '&nbsp;&nbsp;
					</legend>
					<form id="add_server" action="' . $scripturl . '?action=admin;area=packages;get;sa=add" method="post" accept-charset="', $context['character_set'], '">
						<dl class="settings">
							<dt>
				...
				<fieldset>
					<legend>
						<a class="download_by_url">
							<span class="toggle_down floatleft" title="', $txt['show'], '" style="margin-left:-2px;"></span>
						</a>
						' . $txt['package_download_by_url']. '&nbsp;&nbsp;
					</legend>
					<form id="download_url" action="', $scripturl, '?action=admin;area=packages;get;sa=download;byurl;', $context['session_var'], '=', $context['session_id'], '" method="post" accept-charset="', $context['character_set'], '">
						<dl class="settings">
							<dt>
								<strong>' . $txt['serverurl'] . ':</strong>
							</dt>
	...
	else
	{
		...
		foreach ($context['package_list'] as $i => $packageSection)
		{
			...
			foreach ($packageSection['items'] as $id => $package)
			{
				...
				{
					// 1. Some mod [ Download ].
					if($pmxcFunc['strtolower']($package['type']) == 'theme')
						echo '
						<span id="ps_img_', $i, '_pkg_', $id, '" class="toggle_up" alt="*" style="display: none;"></span><strong> ', ($package['can_install'] || !empty($package['can_emulate_install']) ? $package['name'] . ' <a href="' . $package['href'] . '" title="'. $package['name'] .'">[ ' . $txt['download'] . ' ]</a>' : $package['name']), '</strong>
						<ul id="package_section_', $i, '_pkg_', $id, '" class="package_section">';
					else
					{
						echo '
						<span id="ps_img_', $i, '_pkg_', $id, '" class="toggle_up" alt="*" style="display: none;"></span><strong> ', ($package['can_install'] || !empty($package['can_emulate_install']) ? $package['name'] . ' <a href="' . $package['download']['href'] . '" title="'. $package['name'] .'">[ ' . $txt['download'] . ' ]</a>' : $package['name']), '</strong>';

						if(isset($package['is_installed']) && !empty($package['is_installed']))
						{
							echo '
							'. $txt['is_installed'];

							if(isset($package['is_current']) && !empty($package['is_current']))
								echo '
							'. $txt['can_update'];
						}
						echo '
					...
					echo '
							<li class="package_section">', $txt['package_description'], ':&nbsp; ', str_replace('[br]', '<br>', $package['description']), '</li>
						</ul>';
				}
