...
function template_admin()
{
	...
	echo '
							<div id="live_news" class="floatleft">
								...
								<div class="windowbg nopadding verdetais">
	...
	foreach ($context['config_vars'] as $config_var)
	{
		...
		if (is_array($config_var) && ($config_var['type'] == 'title' || $config_var['type'] == 'desc'))
		{
			...
			if ($config_var['type'] == 'title')
			{
				...
				$addMargin = getREQcnt('manageattachments,attachments,antispam,paidsubscribe,logs');
				echo '
							<div class="cat_bar'. ($addMargin > 0 ? ' addtopspace' : '') .'">
								<h3 class="', !empty($config_var['class']) ? $config_var['class'] : 'catbg', '"', !empty($config_var['force_div_id']) ? ' id="' . $config_var['force_div_id'] . '"' : '', '>
									...
									', (isset($config_var['label']) ? $config_var['label'] : ''), '
								</h3>
							</div>';
							$ttlCount++;
			}
			...
			{
				echo '
							<div class="'. (getREQcnt('paidsubscribe') == 0 ? 'information'. (getREQcnt('cache') > 0 ? ' winfo2' : ' winfo') : 'windowbg') .'">
								', (isset($config_var['label']) ? $config_var['label'] : ''), '
							</div>';
			}
			continue;
		...
		if (!$is_open)
		{
			$is_open = true;
			echo '
							<div class="windowbg2', (isset($config_var['name']) && in_array($config_var['name'], array('sef_enabled', 'spider_mode', 'loadavg_enable', 'signature_enable', 'paid_enabled')) ? ' add_top_border' : ''), '">
								<dl class="settings">';
		}
							...
							<div class="windowbg">
								<fieldset>
									...
									<dl class="settings">
										...
										<dd>
											<input type="hidden" name="mlist" value="0" />
	...
	echo '
								...
								<fieldset>
									...
									<dl class="settings">
										...
										<dd>
											<select name="private" id="private" onchange="updateInputBoxes();" style="max-width:95%;">
