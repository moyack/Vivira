...
function template_show_upcoming_list($grid_name)
{
	...
	if (!empty($calendar_data['events']))
	{
		...
		foreach ($calendar_data['events'] as $date => $date_events)
		{
			...
			{
				$event['start_date_local'] = date_format(date_create($event['start_date']), $txt['dp_to_format']);
				$event['end_date_local'] = date_format(date_create($event['end_date']), $txt['dp_to_format']);
				$event['start_time'] = date($txt['dp_timeconv'], strtotime($event['start_time']));
				$event['end_time'] = date($txt['dp_timeconv'], strtotime($event['end_time']));
				...
				else
				{
					...
					echo '<time datetime="' . $event['start_iso_gmdate'] . '">', trim($event['start_date_local']), ', ', trim($event['start_time']), '</time> &ndash; <time datetime="' . $event['end_iso_gmdate'] . '">';
					...
					if ($event['start_date_local'] . $event['start_time_local'] != $event['start_date_orig'] . $event['start_time'])
					{
						...
						echo trim($event['start_time']), '</time> &ndash; <time datetime="' . $event['end_iso_gmdate'] . '">';

						if ($event['start_date_orig'] != $event['end_date_orig'])
							echo trim($event['end_date_orig']) . ', ';

						echo trim($event['end_time']), ' ', $event['tz_abbrev'], '</time>)';
					}
	...
	if (!empty($calendar_data['birthdays']))
	{
		...
		foreach ($calendar_data['birthdays'] as $date)
		{
			...
			$temp = explode(' ', $date[0]['mon_day']);
			if(count($temp) == 2)
				echo sprintf($txt['dp_birstday'], $txt['months_short'][intval($temp[0])], $temp[1]);
	...
	if (!empty($calendar_data['holidays']))
	{
		...
		foreach ($calendar_data['holidays'] as $fulldate => $date)
		{
			unset($date['date_local']);
			$full = explode('-', $fulldate);
			foreach ($date as $holiday)
				$holidays[] = $holiday . ' (' . sprintf($txt['dp_birstday'], $txt['months_short'][intval($full[1])], $full[2]) . ')';
		}
...
function template_calendar_top($calendar_data)
{
	...
	if ($context['calendar_view'] == 'view_list')
	{
		// converting date for the datapicker
		$calendar_data['start_date'] = date_format(date_create_from_format($txt['dp_from_format'], $calendar_data['start_date']), $txt['dp_to_format']);
		$calendar_data['end_date'] = date_format(date_create_from_format($txt['dp_from_format'], $calendar_data['end_date']), $txt['dp_to_format']);

		echo '
			<form action="', $scripturl, '?action=calendar;viewlist" id="calendar_range" method="post" accept-charset="', $context['character_set'], '">
				<input type="text" name="start_date" id="start_date" maxlength="7" size="7" value="', $calendar_data['start_date'], '" tabindex="', $context['tabindex']++, '" class="input_text date_input start" data-type="date">
				<span>', strtolower($txt['calto']), '</span>
				<input type="text" name="end_date" id="end_date" maxlength="7" size="7" value="', $calendar_data['end_date'], '" tabindex="', $context['tabindex']++, '" class="input_text date_input end" data-type="date">
...
function template_event_post()
{
		...
		echo '
					<div class="event_options_right" id="event_board">
						<div>
							...
							<input type="checkbox" style="margin:5px 1% 5px 5%;" class="input_check" name="link_to_board"', (!empty($context['event']['board']) ? ' checked' : ''), ' onclick="toggleLinked(this.form);">
	...
	// converting date for the datapicker
	$context['event']['start_date'] = date_format(date_create_from_format($txt['dp_from_format'], $context['event']['start_date']), $txt['dp_to_format']);
	$context['event']['end_date'] = date_format(date_create_from_format($txt['dp_from_format'], $context['event']['end_date']), $txt['dp_to_format']);
				...
				<fieldset id="event_options">
					...
					<div class="event_options_left" id="event_time_input">
						<div>
							...
							<input type="text" name="start_time" id="start_time" maxlength="10" value="', $context['event']['start_time'], '" tabindex="', $context['tabindex']++, '" class="input_text time_input start" data-type="time"', !empty($context['event']['allday']) ? ' disabled' : '', '>
						</div>
						<div>
							...
							<input type="text" name="end_time" id="end_time" maxlength="10" value="', $context['event']['end_time'], '" tabindex="', $context['tabindex']++, '" class="input_text time_input end" data-type="time"', !empty($context['event']['allday']) ? ' disabled' : '', '>
						</div>
					</div>
	...
	echo '
					...
					<div>
						<span class="label">', $txt['cal_location'], '</span>
						<input type="text" name="event_location" id="event_location" maxlength="255" size="30" value="', !empty($context['event']['location']) ? $context['event']['location'] : '', '" tabindex="', $context['tabindex']++, '" class="input_text">
					</div>
				</fieldset>';
