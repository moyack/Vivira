...
function template_permission_index()
{
			...
			<table class="table_grid">
				<thead>
					<tr class="title_bar">
						...
						<th class="hidden" width="10%" style="text-align:center">', $txt['membergroups_members_top'], '</th>';
	...
	foreach ($context['groups'] as $group)
	{
		...
		echo '
						...
						<td class="hidden">', $group['can_search'] ? $group['link'] : $group['num_members'], '</td>';
...
function template_edit_profiles()
{
	...
	echo '
		...
		<form class="smalltopspace" action="', $scripturl, '?action=admin;area=permissions;sa=profiles" method="post" accept-charset="', $context['character_set'], '">
			<div class="cat_bar">
...
function template_modify_group_display($type)
{
	...
	echo '
				...
				<div class="half_cont_flex">';

	$i = 0;
	foreach ($permission_type['columns'] as $column)
	{
		echo '
					<table class="table_grid half_content half_margin_'. intval($i % 2) .'">';
		$i++;
		foreach ($column as $permissionGroup)
		{
			...
			if (!$permissionGroup['hidden'])
			{
				...
				if ($has_display_content)
				{
					echo '
						<tr class="title_bar">
							<th width="10"></th>
							<th', $context['group']['id'] == -1 ? ' colspan="2"' : '', ' class="smalltext lefttext">', $permissionGroup['name'], '</th>';

					if ($context['group']['id'] != -1)
						echo '
							<th class="smalltext centertext">', $txt['permissions_option_own'], '</th>
							<th class="smalltext centertext">', $txt['permissions_option_any'], '</th>';
				...
				if (!$permission['hidden'] && !$permissionGroup['hidden'])
				{
					...
					if ($permission['has_own_any'])
					{
						...
						if ($context['group']['id'] != -1)
						{
							...
							{
								echo '
								<select class="perm_onoffdeny" name="perm[', $permission_type['id'], '][', $permission['own']['id'], ']" ', $disable_field, '>';
						...
						{
							echo '
								<select class="perm_onoffdeny" name="perm[', $permission_type['id'], '][', $permission['any']['id'], ']" ', $disable_field, '>';
					...
					else
					{
						...
						{
							echo '
								<select class="perm_onoffdeny" name="perm[', $permission_type['id'], '][', $permission['id'], ']" ', $disable_field, '>';
...
function template_postmod_permissions()
{
	...
	echo '
							<div class="windowbg2"'. (!empty($modSettings['postmod_active']) && !empty($modSettings['permission_enable_deny']) ? ' style="border-radius: 0px;"' : '') .'>
								<dl class="settings">
	...
	if (!empty($modSettings['postmod_active']))
	{
		...
		echo '
							<div class="padding" style="padding:5px;min-height:100px;">
								<p class="smalltext">
		...
		echo '
							...
							<div class="div_overflow">
							<table class="table_grid" id="postmod" style="margin-top: 10px;">
								<thead>
									<tr class="title_bar">
										<th></th>
