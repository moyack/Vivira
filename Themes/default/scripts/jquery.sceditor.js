			...
			if (!$.isEmptyObject(this.opts.emoticons.popup)) {
				...
				moreButton = $('<div class="sceditor-more" />').text(this._('More')).on('click', function () {
					...
					if (popup_box.length > 0)
						...
						closeButton = $('<div id="sceditor-popup-close" />').text('[' + popup_close + ']').on('click', function () {
		...
		html: function (element, attrs, content) {
			...
			for (var key in attrs)
			{
				if (key.substr(0, 4) == 'link' && attrs.hasOwnProperty(key) && attrs[key] != '')
				{
					...
					link = (attr_link.substr(0, 7) == 'http://' || attr_link.substr(0, 8) == 'https://') ? attr_link : pmx_scripturl + '?' + attr_link;
				}
			}

			attrs.author = author = typeof(attrs.author) !== "undefined" ? attrs.author : '';
			if (author == '' && sDate == '')
				author = bbc_quote;
