...
var pmx_edit_mode = false;
var pmx_min_screenwidth = 0;
var reloadCalled = false;
var editor_addBR = false;
...
function showimage()
{
	...
	$(document.images.icons).style='display:block';
}
...
function CheckScreenSize()
{
	...
	if(ScrMode != '')
	{
		...
		if(oMode !== cMode && typeof pmx_onForum !== 'undefined' && pmx_onForum == false && typeof have2colblocks !== 'undefined')
		{
			if((cWidth > pmx_colwidth && oWidth < pmx_colwidth) || (oWidth > pmx_colwidth && cWidth < pmx_colwidth))
				ReloadPage();
		}
	}
	else
...
function ReloadPage(skipECL)
{
	if(!pmx_edit_mode && !is_search_robot && (ecl_cache || (skipECL !== 'undefined' && skipECL == true)))
	{
...
/*
 ...
 sType can by 'ecl', 'format', 'cache', 'time', 'sef', 'clr' or empty
...
function pmxCookie(sMode, sName, sValue, sType, bAsync)
{
	...
	notInAllCooks = sName == 'YOfs' || sName == 'screen' || sName == 'spidertest' || sName == 'time' || sName == 'eclauth';
	if(notInAllCooks == false)
	{
	...
	if(sType == 'cache')
	{
		...
		dp = cachedata.match(/[0-9]+\,[0-9]+/g);
		if(dp !== null)
			document.getElementById('cachevals').innerText = cachedata.replace(/[0-9]+\,[0-9]+/g, '0,00');
		else
			document.getElementById('cachevals').innerText = cachedata.replace(/[0-9]+\.[0-9]+/g, '0.00');
	}

	// ecl set and done .. remove ecl overlay and reload page
	if(sName == 'eclauth' && sResult == 'removeECL')
		$('#ecl_outer').fadeOut(800, function(){setTimeout(function(){window.location.reload(true), 800})});
