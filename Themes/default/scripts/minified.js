var pmx_formSubmitted=!1;var lastKeepAliveCheck=new Date().getTime();var pmx_editorArray=new Array();var pop_is_open=!1;var pmx_edit_mode=!1;var pmx_min_screenwidth=0;var reloadCalled=!1;var editor_addBR=!1;var ua=navigator.userAgent.toLowerCase();var is_opera=ua.indexOf('opera')!=-1;var is_ff=(ua.indexOf('firefox')!=-1||ua.indexOf('iceweasel')!=-1||ua.indexOf('icecat')!=-1||ua.indexOf('shiretoko')!=-1||ua.indexOf('minefield')!=-1)&&!is_opera;var is_gecko=ua.indexOf('gecko')!=-1&&!is_opera;var is_chrome=ua.indexOf('chrome')!=-1;var is_safari=ua.indexOf('applewebkit')!=-1&&!is_chrome;var is_webkit=ua.indexOf('applewebkit')!=-1;var is_ie=ua.indexOf('msie')!=-1&&!is_opera;var is_ie11=ua.indexOf('trident')!=-1&&ua.indexOf('gecko')!=-1;var is_iphone=ua.indexOf('iphone')!=-1||ua.indexOf('ipod')!=-1;var is_android=ua.indexOf('android')!=-1;var ajax_indicator_ele=null;if(typeof(pmx_restore_top)=='undefined')
pmx_restore_top=!1;if(!('XMLHttpRequest' in window)&&'ActiveXObject' in window)
...
String.prototype.oCharsetConversion={from:'',to:''};String.prototype.php_to8bit=function(){if(pmx_charset=='UTF-8'){var n,sReturn='';for(var i=0,iTextLen=this.length;i<iTextLen;i++){n=this.charCodeAt(i);if(n<128)
...
String.prototype.php_strtolower=function(){return typeof(pmx_iso_case_folding)=='boolean'&&pmx_iso_case_folding==!0?this.php_strtr('ABCDEFGHIJKLMNOPQRSTUVWXYZ\x8a\x8c\x8e\x9f\xc0\xc1\xc2\xc3\xc4\xc5\xc6\xc7\xc8\xc9\xca\xcb\xcc\xcd\xce\xcf\xd0\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde','abcdefghijklmnopqrstuvwxyz\x9a\x9c\x9e\xff\xe0\xe1\xe2\xe3\xe4\xe5\xe6\xe7\xe8\xe9\xea\xeb\xec\xed\xee\xef\xf0\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8\xf9\xfa\xfb\xfc\xfd\xfe'):this.php_strtr('ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')}
...
return !1;if(pmxCookie('get','spidertest')!='')
return !1;var eclcook=pmxCookie('test','eclauth','','ecl');var sAjax_indicator='<div class="centertext"><img src="'+pmx_images_url+'/loading_sm.gif"></div>';var sIcon=pmx_images_url+'/'+(typeof(sIcon)=='string'?sIcon:'helptopics.png');var sHeader=typeof(sHeader)=='string'?sHeader:help_popup_heading_text;var oContainer=new pmxc_Popup({heading:sHeader,content:sAjax_indicator,icon:sIcon});var oPopup_body=$('#'+oContainer.popup_id).find('.popup_content');$.ajax({url:desktopURL,type:"GET",dataType:"html",beforeSend:function(){},success:function(data,textStatus,xhr){var help_content=$('<div id="temp_help">').html(data).find('a[href$="self.close();"]').hide().prev('br').hide().parent().html();oPopup_body.html(help_content)},error:function(xhr,textStatus,errorThrown){oPopup_body.html((typeof(eclErrorTxt)!='undefined'&&eclcook=='')?eclErrorTxt:textStatus)}});return !1}
function pmxc_PopupMenu(oOptions){this.opt=(typeof oOptions=='object')?oOptions:{};this.opt.menus={}}
pmxc_PopupMenu.prototype.add=function(sItem,sUrl){var $menu=$('#'+sItem+'_menu'),$item=$('#'+sItem+'_menu_top');if($item.length==0)
...
pmxc_PopupMenu.prototype.toggle=function(sItem){if(!!this.opt.menus[sItem].open)
this.close(sItem);else this.open(sItem)}
pmxc_PopupMenu.prototype.open=function(sItem){this.closeAll();if(!this.opt.menus[sItem].loaded){this.opt.menus[sItem].menuObj.html('<div class="loading">'+(typeof(ajax_notification_text)!=null?ajax_notification_text:'')+'</div>');this.opt.menus[sItem].menuObj.load(this.opt.menus[sItem].sUrl,function(){});this.opt.menus[sItem].loaded=!0}
...
pmxc_PopupMenu.prototype.close=function(sItem){this.opt.menus[sItem].menuObj.removeClass('visible');this.opt.menus[sItem].itemObj.removeClass('open');this.opt.menus[sItem].open=!1;$(document).off('click.menu')}
pmxc_PopupMenu.prototype.closeAll=function(){for(var prop in this.opt.menus)
if(!!this.opt.menus[prop].open)
this.close(prop)}
function pmxc_Popup(oOptions){this.opt=oOptions;this.popup_id=this.opt.custom_id?this.opt.custom_id:'pmx_popup';this.show()}
pmxc_Popup.prototype.show=function(){popup_class='popup_window '+(this.opt.custom_class?this.opt.custom_class:'description');if(this.opt.icon_class)
icon='<span class="'+this.opt.icon_class+'"></span> ';else icon=this.opt.icon?'<img src="'+this.opt.icon+'" class="icon" alt=""> ':'';$('body').append('<div id="'+this.popup_id+'" class="popup_container"><div class="'+popup_class+'"><div class="catbg popup_heading"><a href="javascript:void(0);" class="generic_icons hide_popup"></a>'+icon+this.opt.heading+'</div><div class="popup_content">'+this.opt.content+'</div></div></div>');pop_is_open=!0;this.popup_body=$('#'+this.popup_id).children('.popup_window');this.popup_body.parent().fadeIn(300);var popup_instance=this;$(document).keyup(function(e){if(e.keyCode==27)
popup_instance.hide()});$('#'+this.popup_id).find('.hide_popup').click(function(){return popup_instance.hide()});return !1}
pmxc_Popup.prototype.hide=function(){$('#'+this.popup_id).fadeOut(300,function(){$(this).remove()});pop_is_open=!1;return !1}
...
function submitonce(theform){pmx_formSubmitted=!0;for(var i=0;i<pmx_editorArray.length;i++)
...
aTextareas[i].readOnly=!0;return !pmx_formSubmitted}
...
var lastKeepAliveCheck=new Date().getTime();function pmx_sessionKeepAlive(){var curTime=new Date().getTime();if(pmx_scripturl&&curTime-lastKeepAliveCheck>900000){var tempImage=new Image();tempImage.src=pmx_prepareScriptUrl(pmx_scripturl)+'action=keepalive;time='+curTime;lastKeepAliveCheck=curTime}
window.setTimeout('pmx_sessionKeepAlive();',1200000)}
window.setTimeout('pmx_sessionKeepAlive();',1200000);function pmx_setThemeOption(theme_var,theme_value,theme_id,theme_cur_session_id,theme_cur_session_var,theme_additional_vars){if(theme_cur_session_id==null)
theme_cur_session_id=pmx_session_id;if(typeof(theme_cur_session_var)=='undefined')
theme_cur_session_var='sesc';if(theme_additional_vars==null)
theme_additional_vars='';var tempImage=new Image();tempImage.src=pmx_prepareScriptUrl(pmx_scripturl)+'action=jsoption;var='+theme_var+';val='+theme_value+';'+theme_cur_session_var+'='+theme_cur_session_id+theme_additional_vars+(theme_id==null?'':'&th='+theme_id)+';time='+(new Date().getTime())}
...
function pmxc_preCacheImage(sSrc){if(!('pmxc_aCachedImages' in window))
window.pmxc_aCachedImages=[];if(!in_array(sSrc,window.pmxc_aCachedImages)){var oImage=new Image();oImage.src=sSrc}}
function pmxc_Toggle(oOptions){this.opt=oOptions;this.bCollapsed=!1;this.oCookie=null;this.init()}
pmxc_Toggle.prototype.init=function(){if('bToggleEnabled' in this.opt&&!this.opt.bToggleEnabled)
return;if('oCookieOptions' in this.opt&&this.opt.oCookieOptions.bUseCookie){var cookieValue=pmxCookie('get',this.opt.oCookieOptions.sCookieName);this.opt.bCurrentlyCollapsed=cookieValue=='1'}
...
else{pmxc_preCacheImage(this.opt.aSwapImages[i].srcCollapsed)}
...
pmxc_Toggle.prototype.changeState=function(bCollapse,bInit){bInit=typeof(bInit)=='undefined'?!1:!0;if(!bInit&&bCollapse&&'funcOnBeforeCollapse' in this.opt){this.tmpMethod=this.opt.funcOnBeforeCollapse;this.tmpMethod();delete this.tmpMethod}
...
continue;var oContainer=document.getElementById(this.opt.aSwappableContainers[i]);if(typeof(oContainer)=='object'&&oContainer!=null){if('oCookieOptions' in this.opt&&'sCookieName' in this.opt.oCookieOptions&&this.opt.oCookieOptions.sCookieName.match(/Panel/)&&(this.opt.oCookieOptions.sCookieName.match(/left/)||this.opt.oCookieOptions.sCookieName.match(/right/))){if(typeof portamx_EqualHeight=='function'&&oContainer.id!='upshrinkVisual'){if(bCollapse)
$(oContainer).hide(400,function(){portamx_EqualHeight(!0)});else $(oContainer).show(400,function(){portamx_EqualHeight(!0)})}
else{if(bCollapse)
$(oContainer).hide(400);else $(oContainer).show(400)}
if(typeof(PmxOpacFader)!=='undefined')
setTimeout(function(){NewFaderHeight()},500)}
else{if(bInit)
$(oContainer).toggle(!bCollapse);else{if(bCollapse){if(typeof portamx_EqualHeight=='function'&&oContainer.id!='upshrinkVisual')
$(oContainer).slideUp(400,function(){portamx_EqualHeight(null)});else $(oContainer).slideUp(400)}
else{if(typeof portamx_EqualHeight=='function'&&oContainer.id!='upshrinkVisual')
$(oContainer).slideDown(400,function(){portamx_EqualHeight(null)});else $(oContainer).slideDown(400)}}}}}
this.bCollapsed=bCollapse;if('oCookieOptions' in this.opt&&this.opt.oCookieOptions.bUseCookie)
pmxCookie('set',this.opt.oCookieOptions.sCookieName,this.bCollapsed|0,'',!0);if(!bInit&&'oThemeOptions' in this.opt&&this.opt.oThemeOptions.bUseThemeSettings)
...
pmxc_Toggle.prototype.toggle=function(){this.changeState(!this.bCollapsed)}
...
function createEventListener(oTarget){if(typeof(oTarget)=='undefined')
return;if(!('addEventListener' in oTarget)){if(oTarget.attachEvent){oTarget.addEventListener=function(sEvent,funcHandler,bCapture){oTarget.attachEvent('on'+sEvent,funcHandler)}
...
function grabJumpToContent(elem){var oXMLDoc=getXMLDocument(pmx_prepareScriptUrl(pmx_scripturl)+'action=xmlhttp;sa=jumpto;xml');var aBoardsAndCategories=new Array();var bIE5x=!('implementation' in document);ajax_indicator(!0);if(oXMLDoc.responseXML){var items=oXMLDoc.responseXML.getElementsByTagName('pmx')[0].getElementsByTagName('item');for(var i=0,n=items.length;i<n;i++){aBoardsAndCategories[aBoardsAndCategories.length]={id:parseInt(items[i].getAttribute('id')),isCategory:items[i].getAttribute('type')=='category',name:items[i].firstChild.nodeValue.removeEntities(),is_current:!1,childLevel:parseInt(items[i].getAttribute('childlevel'))}}}
...
IconList.prototype.onBoxHover=function(oDiv,bMouseOver){oDiv.style.border=bMouseOver?this.opt.iBoxBorderWidthHover+'px solid '+this.opt.sBoxBorderColorHover:'';oDiv.style.background=bMouseOver?this.opt.sBoxBackgroundHover:this.opt.sBoxBackground;oDiv.style.padding=bMouseOver?'2px 2px 0px 2px':'3px 3px 1px 3px'}
IconList.prototype.openPopup=function(oDiv,iMessageId){this.iCurMessageId=iMessageId;if(!this.bListLoaded&&this.oContainerDiv==null){this.oContainerDiv=document.createElement('div');this.oContainerDiv.id='iconList';this.oContainerDiv.style.display='none';this.oContainerDiv.style.cursor='pointer';this.oContainerDiv.style.position='absolute';this.oContainerDiv.style.background=this.opt.sContainerBackground;this.oContainerDiv.style.border=this.opt.sContainerBorder;this.oContainerDiv.style.padding='2px 1px 4px 1px';document.body.appendChild(this.oContainerDiv);ajax_indicator(!0);sendXMLDocument.call(this,pmx_prepareScriptUrl(pmx_scripturl)+'action=xmlhttp;sa=messageicons;board='+this.opt.iBoardId+';xml','',this.onIconsReceived);createEventListener(document.body)}
this.oContainerDiv.style.top=$(oDiv).offset().top+24+'px';this.oContainerDiv.style.left=$(oDiv).offset().left+'px';this.oClickedIcon=oDiv;if(this.bListLoaded)
this.oContainerDiv.style.display='block';document.body.addEventListener('mousedown',this.onWindowMouseDown,!1)}
IconList.prototype.onIconsReceived=function(oXMLDoc){var icons=oXMLDoc.getElementsByTagName('pmx')[0].getElementsByTagName('icon');var sItems='';for(var i=0,n=icons.length;i<n;i++)
...
IconList.prototype.onItemHover=function(oDiv,bMouseOver){oDiv.style.background=bMouseOver?this.opt.sItemBackgroundHover:this.opt.sItemBackground;oDiv.style.border=bMouseOver?this.opt.sItemBorderHover:this.opt.sItemBorder;oDiv.style.padding='1px 3px 5px';if(this.iCurTimeout!=0)
...
IconList.prototype.onItemMouseDown=function(oDiv,sNewIcon){if(this.iCurMessageId!=0){ajax_indicator(!0);$.ajax({type:'GET',async:!1,url:pmx_scripturl+'?action=jsmodify;topic='+this.opt.iTopicId+';msg='+this.iCurMessageId+';'+pmx_session_var+'='+pmx_session_id+';icon='+sNewIcon+';xml',data:{},success:function(data){sResult=data}});ajax_indicator(!1);var oMessage=sResult.getElementsByTagName('pmx')[0].getElementsByTagName('message')[0];if(oMessage.getElementsByTagName('error').length==0)
...
function pmx_mousePose(oEvent){var x=0;var y=0;if(oEvent.pageX){y=oEvent.pageY;x=oEvent.pageX}
...
return[x,y]}
function pmx_itemPos(itemHandle){var itemX=0;var itemY=0;if('offsetParent' in itemHandle){itemX=itemHandle.offsetLeft;itemY=itemHandle.offsetTop;while(itemHandle.offsetParent&&typeof(itemHandle.offsetParent)=='object'){itemHandle=itemHandle.offsetParent;itemX+=itemHandle.offsetLeft;itemY+=itemHandle.offsetTop}}
else if('x' in itemHandle){itemX=itemHandle.x;itemY=itemHandle.y}
return[itemX,itemY]}
function pmx_prepareScriptUrl(sUrl){return sUrl.indexOf('?')==-1?sUrl+'?':sUrl}
...
function pmxFooterHighlight(element,value){element.src=pmx_images_url+'/'+(value?'h_':'')+element.id+'.png'}
function pmxSelectText(oCurElement,bActOnElement){if(typeof(bActOnElement)=='boolean'&&bActOnElement)
...
else if(window.getSelection){var oCurSelection=window.getSelection();if(oCurSelection.selectAllChildren){var oLastChild=oCodeArea.lastChild;oCurSelection.setBaseAndExtent(oCodeArea,0,oLastChild,'innerText' in oLastChild?oLastChild.innerText.length:oLastChild.textContent.length)}
...
return !1}
function pmxc_saveEntities(sFormName,aElementNames,sMask){if(typeof(sMask)=='string'){for(var i=0,n=document.forms[sFormName].elements.length;i<n;i++)
...
function showimage(){$(document.images.icons).style='display:none';document.images.icons.src=icon_urls[document.forms.postmodify.icon.options[document.forms.postmodify.icon.selectedIndex].value];$(document.images.icons).style='display:block'}
...
$(item).css('display','block');$(item).css('top',$(this).offset().top+$(this).height());$(item).css('left',Math.max($(this).offset().left-$(item).width()+$(this).outerWidth(),0));$(item).height($(item).find('div:first').height())});$(document).click(function(){$(item).css('display','none')})});$(document).on('click','.you_sure',function(){var custom_message=$(this).attr('data-confirm');return confirm(custom_message?custom_message.replace(/-n-/g,"\n"):pmx_you_sure)});$('.pmx_select_text').on('click',function(e){e.preventDefault();var actOnElement=$(this).attr('data-actonelement');return typeof actOnElement!=="undefined"?pmxSelectText(actOnElement,!0):pmxSelectText(this)})});function sysOnLoad(){showPage=$('.pages').css('display');Curr='#';if($('.current_page')[0])
Curr=$('.current_page')[0].innerText;if(showPage=='none')
if(Curr=='1')
$('.current_page').css({'border-top-left-radius':'3px','border-bottom-left-radius':'3px'});if(Curr=='#'){$('.all').removeClass('navPages').addClass('navPages_sel');$('.pagelinks a:last-of-type').css({'display':'none'})}
if(mobile_device==!0){a=0;for(i=0;i<100;i++){};CheckScreenSize();if(reloadCalled)
return}
fSetContentHeight()}
function CheckScreenSize(){reloadCalled=!1;if(window.screen.availHeight>window.screen.availWidth)
cMode='Portrait';else cMode='Landscape';cWidth=window.screen.availWidth;ScrMode=pmxCookie('get','screen',cMode+'-'+cWidth,'set');if(ScrMode!=''){tmp=ScrMode.split('-');oMode=tmp[0];oWidth=parseInt(tmp[1]);if(oMode!==cMode&&typeof pmx_onForum!=='undefined'&&pmx_onForum==!1&&typeof have2colblocks!=='undefined'){if((cWidth>pmx_colwidth&&oWidth<pmx_colwidth)||(oWidth>pmx_colwidth&&cWidth<pmx_colwidth))
ReloadPage()}}
else ReloadPage(!0)}
function ReloadPage(skipECL){if(!pmx_edit_mode&&!is_search_robot&&(ecl_cache||(skipECL!=='undefined'&&skipECL==!0))){setTimeout(function(){window.location.reload(!0)});document.getElementsByTagName('BODY')[0].style.opacity='0.0';reloadCalled=!0}}
function pmxCookie(sMode,sName,sValue,sType,bAsync){notInAllCooks=sName=='YOfs'||sName=='screen'||sName=='spidertest'||sName=='time'||sName=='eclauth';if(notInAllCooks==!1){if(typeof(allCookies)!=='undefined'){if(sMode=='get'&&allCookies.Name.indexOf(sName)>=0)
return allCookies.Value[allCookies.Name.indexOf(sName)];else if(sMode=='set'){if(allCookies.Name.indexOf(sName)>=0){if(allCookies.Value[allCookies.Name.indexOf(sName)]!==sValue.toString()){allCookies.Value[allCookies.Name.indexOf(sName)]=sValue.toString();bAsync=!0}
else return allCookies.Value[allCookies.Name.indexOf(sName)]}
else{allCookies.Name[allCookies.Name.length]=sName;allCookies.Value[allCookies.Value.length]=(sValue==undefined?'':sValue.toString());bAsync=!0}}}}
sValue=sValue==undefined?'':sValue;bAsync=bAsync==undefined?!1:bAsync;sType=sType==undefined?'':sType;var sResult='';$.ajax({type:'GET',async:bAsync,url:pmx_scripturl+'?jscook',data:{mode:sMode,name:sName,value:sValue,type:sType},success:function(data){sResult=data}});if(sType=='cache'){var cachedata=document.getElementById('cachevals').innerText;dp=cachedata.match(/[0-9]+\,[0-9]+/g);if(dp!==null)
document.getElementById('cachevals').innerText=cachedata.replace(/[0-9]+\,[0-9]+/g,'0,00');else document.getElementById('cachevals').innerText=cachedata.replace(/[0-9]+\.[0-9]+/g,'0.00')}
if(sName=='eclauth'&&sResult=='removeECL'){$('#ecl_outer').fadeOut(500);return ''}
return sResult};$(document).ready(function(){$('ul.dropmenu, ul.quickbuttons').superfish({delay:250,speed:100,sensitivity:8,interval:50,timeout:1});$('.preview').PMXtooltip();$('a.bbc_link img.bbc_img').parent().css('border','opera')});function pmx_codeBoxFix(){var codeFix=$('code');$.each(codeFix,function(index,tag){if(is_webkit&&$(tag).height()<20)
...
addLoadEvent(pmx_codeBoxFix);(function($){$.fn.PMXtooltip=function(oInstanceSettings){$.fn.PMXtooltip.oDefaultsSettings={followMouse:1,hoverIntent:{sensitivity:10,interval:300,timeout:50},positionTop:12,positionLeft:12,tooltipID:'pmx_tooltip',tooltipTextID:'pmx_tooltipText',tooltipClass:'tooltip',tooltipSwapClass:'pmx_swaptip',tooltipContent:'html'};var oSettings=$.extend({},$.fn.PMXtooltip.oDefaultsSettings,oInstanceSettings||{});$(this).each(function(){var sTitle=$('<span class="'+oSettings.tooltipSwapClass+'">'+htmlspecialchars(this.title)+'</span>').hide();$(this).append(sTitle).attr('title','')});var positionTooltip=function(event){var iPosx=0;var iPosy=0;if(!event)
...
return this.each(function(index){if($.fn.hoverIntent){$(this).hoverIntent({sensitivity:oSettings.hoverIntent.sensitivity,interval:oSettings.hoverIntent.interval,over:pmx_tooltip_on,timeout:oSettings.hoverIntent.timeout,out:pmx_tooltip_off})}
else{$(this).hover(pmx_tooltip_on,pmx_tooltip_off)}
function pmx_tooltip_on(event){if($(this).children('.'+oSettings.tooltipSwapClass).text()){$('body').append('<div id="'+oSettings.tooltipID+'" class="'+oSettings.tooltipClass+'"><div id="'+oSettings.tooltipTextID+'" style="display:none;"></div></div>');var tt=$('#'+oSettings.tooltipID);var ttContent=$('#'+oSettings.tooltipID+' #'+oSettings.tooltipTextID);if(oSettings.tooltipContent=='html')
...
return !1};function pmx_tooltip_off(event){hideTooltip(this);return !1};if(oSettings.followMouse){$(this).on("mousemove",function(event){positionTooltip(event);return !1})}
...
if(e.type=="mouseenter"){pX=ev.pageX;pY=ev.pageY;$(ob).on("mousemove.hoverIntent",track);if(ob.hoverIntent_s!=1){ob.hoverIntent_t=setTimeout(function(){compare(ev,ob)},cfg.interval)}}else{$(ob).off("mousemove.hoverIntent",track);if(ob.hoverIntent_s==1){ob.hoverIntent_t=setTimeout(function(){delay(ev,ob)},cfg.timeout)}}};return this.on({'mouseenter.hoverIntent':handleHover,'mouseleave.hoverIntent':handleHover},cfg.selector)}})(jQuery);$(function(){if(pmx_member_id>0)
$('div.boardindex_table div.cat_bar').each(function(index,el){var catid=el.id.replace('category_','');new pmxc_Toggle({bToggleEnabled:!0,bCurrentlyCollapsed:$('#category_'+catid+'_upshrink').data('collapsed'),aSwappableContainers:['category_'+catid+'_boards'],aSwapImages:[{sId:'category_'+catid+'_upshrink',msgExpanded:'',msgCollapsed:''}],oThemeOptions:{bUseThemeSettings:!0,sOptionName:'collapse_category_'+catid,sSessionVar:pmx_session_var,sSessionId:pmx_session_id}})})});$(function(){$('.mobile_act').click(function(){$('#mobile_action').show()});$('.hide_popup').click(function(){$('#mobile_action').hide()});$('.mobile_mod').click(function(){$('#mobile_moderation').show()});$('.hide_popup').click(function(){$('#mobile_moderation').hide()});$('.mobile_user_menu').click(function(){$('#mobile_user_menu').show()});$('.hide_popup').click(function(){$('#mobile_user_menu').hide()})})