	...
	if (isset($_GET['save']))
	{
		...
		if (!empty($_POST['use_subdirectories_for_attachments']))
		{
			...
			if (!empty($_POST['use_subdirectories_for_attachments']) && !empty($_POST['basedirectory_for_attachments']) && !in_array($_POST['basedirectory_for_attachments'], $modSettings['attachment_basedirectories']))
			{
				...
				if (is_array($modSettings['attachmentUploadDir']) && !in_array($_POST['basedirectory_for_attachments'], $modSettings['attachmentUploadDir']))
				{
				...
				if (is_array($modSettings['attachmentUploadDir']) && !in_array($_POST['basedirectory_for_attachments'], $modSettings['attachment_basedirectories']))
				{
				...
				'data' => array(
					'function' => function ($rowData) use ($modSettings, $context, $scripturl, $txt, $pmxcFunc)
					{
						...
						if($expand)
						{
							...
							if ($rowData['attachment_type'] == 1)
								$link .= '" class="lb-link-inl" title="'. $txt['lightbox_expand_short'] .'" data-title=" " data-lightbox="manage-avatar';

							else if (!empty($rowData['width']) && !empty($rowData['height']))
							{
								if ($rowData['attachment_type'] == 3)
									$link .= ';image" class="lb-link-inl" title="'. $txt['lightbox_expand_short'] .'"  data-title=" " data-lightbox="manage-avatar';
								else
									$link .= ';image" class="lb-link-inl" title="'. $txt['lightbox_expand_short'] .'"  data-title="'. $pmxcFunc['htmlspecialchars']($rowData['filename']) .'" data-lightbox="manage-attach';
							}
						...
						if (!empty($rowData['width']) && !empty($rowData['height']))
							$link .= sprintf('<br><span class="smalltext">(%1$dx%2$d)</span>', $rowData['width'], $rowData['height']);
...
function ManageAttachmentPaths()
{
	...
	if (isset($_REQUEST['save2']))
	{
		...
		{
			if (!isset($modSettings['basedirectory_for_attachments']) || (isset($modSettings['basedirectory_for_attachments']) && $modSettings['basedirectory_for_attachments'] != $modSettings['attachmentUploadDir'][$_POST['current_base_dir']]))
				$update = (array(
	...
	$listOptions = array(
		...
		'columns' => array(
			...
			'path' => array(
				...
				'data' => array(
					'function' => function ($rowData)
					{
						return '<input type="hidden" name="dirs[' . $rowData['id'] . ']" value="' . $rowData['path'] . '"><input type="text" size="54" name="dirs[' . $rowData['id'] . ']" value="' . $rowData['path'] . '"' . (!empty($rowData['disable_base_dir']) ? ' disabled' : '') . ' class="input_text">';
					},
					'style' => 'width: 40%;',
				),
			),
		...
		'additional_rows' => array(
			...
			empty($errors['dir']) ? array(
				...
				'style' => 'padding: 5px 10px;width:auto;width:-moz-available;width:-webkit-fill-available;width:fill-available;',
				'class' => 'windowbg2 smalltext'
			) : array(
				'position' => 'top_of_list',
				'value' => $txt['attach_dir_save_problem'] . '<br>' . implode('<br>', $errors['dir']),
				'style' => 'padding-left: 35px;width:auto;width:-moz-available;width:-webkit-fill-available;width:fill-available;display:flex;float:unset;',
				'class' => 'noticebox',
			),
		),
	);
...
function list_getBaseDirs()
{
	...
	foreach ($modSettings['attachment_basedirectories'] as $id => $dir)
	{
		...
		$basedirs[] = array(
			...
			'path' => $expected_dirs > 0 ? $dir : ('<input type="text" name="base_dir[' . $id . ']" value="' . $dir . '" size="54">'),
	...
	if (isset($_REQUEST['new_base_path']))
		$basedirs[] = array(
			'id' => '',
			'current' => false,
			'path' => '<input type="text" name="new_base_dir" value="" size="60">',
			'num_dirs' => '',
			'status' => '',
		);
