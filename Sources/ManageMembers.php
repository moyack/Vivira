...
function ViewMemberlist()
{
	...
	if ($context['sub_action'] == 'query')
	{
		...
		$params = array(
			...
			'gdpraccepted' => array(
				'db_fields' => array('gdpr_date'),
				'type' => 'checkbox',
				'values' => array('0', strval(strtotime($modSettings['gdpr_last_update']) + 43200)),
			),
			'membername' => array(
		...
		{
			// Not filled in?
			if (!isset($search_params[$param_name]) || empty($search_params[$param_name]))
				continue;
			...
			elseif ($param_info['type'] == 'checkbox')
			{
				...
				$query_parts[] = $param_info['db_fields'][0] . ' IN ({array_string:' . $param_name . '_check})'. ($param_name == 'gdpraccepted' ? (intval($search_params['gdpraccepted'][0]) > 0 ? ' OR mem.id_group = 1 OR mem.additional_groups = 1' : ' AND mem.id_group != 1 AND mem.additional_groups != 1') : '');
...
function AdminApprove()
{
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		...
		$member_info[] = array(
			...
			'language' => empty($row['lngfile']) ? $language : $row['lngfile'],
