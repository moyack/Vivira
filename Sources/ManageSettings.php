...
{
	global $context, $txt, $pmxCacheFunc;
...
{
	global $txt, $scripturl, $context, $modSettings, $pmxCacheFunc, $cache_enable, $user_info, $txt;
...
//	$pmxCacheFunc['clean']();
	...
	$config_vars = array(
		...
		'',
			// the (ugly) GDPR of the EU
			array('check', 'gdpr_enabled'),
			array('text', 'gdpr_owner_name', null, 'size' => 40),
			array('text', 'gdpr_owner_street', null, 'size' => 40),
			array('text', 'gdpr_owner_location', null, 'size' => 40),
			array('text', 'gdpr_owner_country', null, 'size' => 40),
			array('text', 'gdpr_owner_email', null, 'size' => 40),
			array('text', 'gdpr_owner_hoster', null, 'size' => 100),
			array('text', 'gdpr_last_update', null,'size' => 8, 'subtext' => $txt['gdpr_format_note']),
			array('check', 'imprint_enabled'),
		'',
			// Big Options... ecl, polls, sticky, bbc....
			array('check', 'ecl_enabled', 0, 'disabled' => empty($cache_enable)),
			array('check', 'ecl_nomodal'),
			array('check', 'ecl_nomodal_mobile'),
			array('int', 'ecl_topofs', 'step' => 1, 'min' => 0, 'max' => 999),
		'',
			array('check', 'lang_autodetect', 0, 'disabled' => empty($cache_enable)),
			array('check', 'geoip_enabled', 0),
			array('text', 'geoip_sslkey', 'size' => 15),
			array('check', 'langdetect_log'),
		'',
			array('text', 'google_site_verification', 'subtext' => $txt['google_site_verification_hlp'], 'size' => 50),
		'',
			array('check', 'webkit_scrollbars'),
			array('check', 'add_favicon_to_links'),
			array('check', 'dont_use_lightbox'),
			array('check', 'image_download'),
			array('check', 'image_addwatermark'),
			array('text', 'watermark_image', 'size' => 24),
		'',
		...
		'',
			...
			array('check', 'minimize_files', 'disabled' => empty($cache_enable)),
			array('check', 'queryless_urls', 'disabled' => !empty($modSettings['sef_enabled']), 'subtext' => '<strong>' . $txt['queryless_urls_note'] . '</strong>'),
	...
	if (isset($_GET['save']))
	{
		...
		// check and fixup ecl countrys
		if(isset($_POST['ecl_countrys']))
		{
			$tmp = preg_split('/[\s,]+/', strtoupper($_POST['ecl_countrys']));
			sort($tmp, SORT_STRING);
			$_POST['ecl_countrys'] = implode(',', $tmp);
		}

		if(!isset($_POST['ecl_enabled']) && empty($cache_enable))
			$_POST['ecl_enabled'] = 0;
		...
		if(isset($_POST['ecl_enabled']) && $_POST['ecl_enabled'] == '1' && !checkECL_Cookie())
			setECL_Cookie();
...
function ModifyLayoutSettings($return_config = false)
{
	...
	$config_vars = array(
		...
		'',
			...
			array('select', 'todayMod', array(0 => $txt['today_disabled'], 1 => $txt['today_only'], 2 => $txt['yesterday_today'], 3 => $txt['relative_time_date'])),
	...
	if (!empty($modSettings['warning_settings']) && $currently_enabled)
		$config_vars += array(
			'',
				...
				array('permissions', 'view_warning', 'subtext' => $txt['setting_view_warning_note']),
		);
...
function ModifyAntispamSettings($return_config = false)
{
	...
	$config_vars = array(
				...
				array('check', 'guests_require_captcha'),
			...
			array('text', 'recaptcha_public_key', 50),
			array('text', 'recaptcha_private_key', 50),
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		...
		$context['question_answers'][$row['id_question']] = array(
			'lngfile' => !empty($lang) ? $lang : $language,
	...
	if (isset($_GET['save']))
	{
		...
		if(!empty($_POST['recaptcha_enabled']))
			$_POST['visual_verification_type'] = '';
...
function ShowCustomProfiles()
{
	...
	$standard_fields = array('website', 'personal_text', 'timezone', 'posts', 'warning_status', 'gender', 'location');
...
function list_getProfileFields($start, $items_per_page, $sort, $standardFields)
{
	...
	else
	{
		...
		while ($row = $pmxcFunc['db_fetch_assoc']($request))
		{
			$row['field_desc'] = $txt[$row['col_name']];
			$list[] = $row;
		}
		$pmxcFunc['db_free_result']($request);
	}
