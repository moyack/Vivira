...
function getBoardIndex($boardIndexOptions)
{
	...
	$result_boards = $pmxcFunc['db_query']('boardindex_fetch_boards', '
		SELECT' . ($boardIndexOptions['include_categories'] ? '
			...
			c.can_collapse,' : '')) . '
			COALESCE(mem.id_member, 0) AS id_member, mem.avatar, m.id_msg' . (!empty($modSettings['avatars_on_boardIndex']) ? ',  mem.email_address, mem.avatar, COALESCE(am.id_attach, 0) AS member_id_attach, am.filename AS member_filename, am.attachment_type AS member_attach_type' : '') . '
		FROM {db_prefix}boards AS b' . ($boardIndexOptions['include_categories'] ? '
			LEFT JOIN {db_prefix}categories AS c ON (c.id_cat = b.id_cat)' : '') . '
			LEFT JOIN {db_prefix}messages AS m ON (m.id_msg = b.id_last_msg)
			LEFT JOIN {db_prefix}members AS mem ON (mem.id_member = m.id_member)' . (!empty($modSettings['avatars_on_boardIndex']) ? '
	...
	while ($row_board = $pmxcFunc['db_fetch_assoc']($result_boards))
	{
		...
		if ($boardIndexOptions['include_categories'])
		{
			...
			if (empty($categories[$row_board['id_cat']]))
			{
				$categories[$row_board['id_cat']] = array(
					...
					'new' => false,
					'css_class' => '',
					'tm_mode' => $user_info['tmdisplay'] == 3 ? 'rel' : '',
				);
		...
		{
			$this_last_post['href'] = $scripturl . '?topic=' . $row_board['id_topic'] . '.msg'. $row_board['id_msg'] . (empty($row_board['is_read']) ? ';boardseen' : '') . '#msg'. $row_board['id_msg'];
