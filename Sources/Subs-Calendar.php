...
{
	global $scripturl, $modSettings, $user_info, $pmxcFunc, $context, $sourcedir;
...
function getCalendarWeek($month, $year, $day, $calendarOptions)
{
	...
	if (!empty($calendarOptions['show_week_num']))
	{
		...
		$calendarGrid['week_title'] = sprintf($txt['calendar_week_beginning'], $txt['months_titles'][date('n', $timestamp)], date('j', $timestamp), date('Y', $timestamp));
	}
...
{
	global $scripturl, $modSettings, $user_info, $txt, $context, $options, $sourcedir;
	require_once($sourcedir . '/Subs.php');
	...
	if (preg_match('~%[AaBbCcDdeGghjmuYy](?:[^%]*%[AaBbCcDdeGghjmuYy])*~', $modSettings['time_format'], $matches) == 0 || empty($matches[0]))
		$date_format = '%b %d';
	else
		$date_format = str_replace(array('%Y', '%y', '%G', '%g', '%C', '%c', '%D'), array('', '', '', '', '', '%b %d', '%m/%d'), $matches[0]);

	foreach (array('holidays') as $type)
	{
	...
	loadCSSFile('jquery-ui.datepicker.css', array('defer' => false), 'pmx_datepicker');
	loadJavaScriptFile('jquery-ui.datepicker.min.js', array('defer' => true), 'pmx_datepicker');
	loadJavaScriptFile('jquery.timepicker.min.js', array('defer' => true), 'pmx_timepicker');
	loadJavaScriptFile('datepair.min.js', array('defer' => true), 'pmx_datepair');
	addInlineJavaScript('
	$("#calendar_range .date_input").datepicker({
		dateFormat: "'. $txt['dp_format'] .'",
		firstDay: '. intval($user_info['is_guest'] ? $txt['dp_firstday'] : (!empty($options['calendar_start_day']) ? $options['calendar_start_day'] : 0)) .',
		autoSize: true,
		isRTL: ' . ($context['right_to_left'] ? 'true' : 'false') . ',
		constrainInput: true,
		showAnim: "",
		showButtonPanel: false,
		minDate: "'. sprintf($txt['dp_minDate'], $modSettings['cal_minyear']) . '",
		maxDate: "'. sprintf($txt['dp_minDate'], $modSettings['cal_maxyear']) . '",
...
function validateEventPost()
{
	...
	{
		// Convert am/pm Time
		if(isset($_POST['start_time']) && preg_match('/AM|PM|am|pm/', $_POST['start_time']) > 0)
		{
			$sTM = date_create_from_format('H:i: A', $_POST['start_time']);
			$_POST['start_time'] = $sTM->format('H:i') .':';
		}
		if(isset($_POST['end_time']) && preg_match('/AM|PM|am|pm/', $_POST['end_time']) > 0)
		{
			$eTM = date_create_from_format('H:i: A', $_POST['end_time']);
			$_POST['end_time'] = $eTM->format('H:i') .':';
		}

		if (isset($_POST['start_date']))
		{
			if(isset($_POST['start_time']))
				$d = date_parse($_POST['start_date'] .' '. trim($_POST['start_time'], ':'));
			else
...
function insertEvent(&$eventOptions)
{
	...
	$eventOptions['location'] = isset($eventOptions['location']) ? $pmxcFunc['htmlspecialchars']($eventOptions['location'], ENT_QUOTES) : '';
	...
	// Insert the event!
	$eventOptions['id'] = $pmxcFunc['db_insert']('',
		'{db_prefix}calendar',
		$event_columns,
		$event_parameters,
		array('id_event'),
...
function modifyEvent($event_id, &$eventOptions)
{
	...
	$eventOptions['title'] = $pmxcFunc['htmlspecialchars']($eventOptions['title'], ENT_QUOTES);
	$eventOptions['location'] = $pmxcFunc['htmlspecialchars']($eventOptions['location'], ENT_QUOTES);
...
function setEventStartEnd($eventOptions = array())
{
	...
	if (empty($start_string) && isset($start_date_string))
		$start_string = $start_date_string . (isset($start_time_string) ? ' ' . trim($start_time_string, ':') : '');
	if (empty($end_string) && isset($end_date_string))
		$end_string = $end_date_string . (isset($end_time_string) ? ' ' . trim($end_time_string, ':') : '');
...
{
	global $sourcedir, $user_info, $txt, $modSettings;
	require_once($sourcedir . '/Subs.php');
	...
	if (preg_match('~%[HkIlMpPrRSTX](?:[^%]*%[HkIlMpPrRSTX])*~', $modSettings['time_format'], $matches) == 0 || empty($matches[0]))
		$time_format = '%k:%M';
	else
		$time_format = str_replace(array('%I', '%S', '%r', '%R', '%T'), array('%l', '', '%l:%M %p', '%k:%M', '%l:%M'), $matches[0]);
	...
	$start = date_parse($row['start_date'] . (empty($allday) ? ' ' . $row['start_time'] : ''));
	$end = date_parse($row['end_date'] . (empty($allday) ? ' ' . $row['end_time'] : ''));

	// But we also want more info, so make some DateTime objects we can use
	$start_object = date_create($row['start_date'] . (empty($allday) ? ' ' . $row['start_time'] : ''), timezone_open($row['timezone']));
	$end_object = date_create($row['end_date'] . (empty($allday) ? ' ' . $row['end_time'] : ''), timezone_open($row['timezone']));
...
{
	global $pmxcFunc, $context, $sourcedir, $user_info, $modSettings;
	...
	if (isset($id_member))
	{
		...
		list($timezone) = $pmxcFunc['db_fetch_row']($request);
		$pmxcFunc['db_free_result']($request);
	}
