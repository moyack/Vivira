...
function RemindPick()
{
	...
	loadLanguage('LangSettings', $row['lngfile'], false);
	loadLanguage('index', $row['lngfile'], false);
	...
	if (empty($row['secret_question']) || (isset($_POST['reminder_type']) && $_POST['reminder_type'] == 'email'))
	{
		...
		$emaildata = loadEmailTemplate('forgot_password', $replacements, empty($row['lngfile']) ? $language : $row['lngfile']);
