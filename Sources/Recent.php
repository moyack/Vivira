	...
	if ($context['showing_all_topics'] && $have_temp_table)
	{
		...
		$request = $pmxcFunc['db_query']('substring', '
			...
			FROM {db_prefix}messages AS ms
				...
				LEFT JOIN {db_prefix}members AS meml ON (meml.id_member = ml.id_member)' . (!empty($modSettings['avatars_on_boardIndex']) ? '
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		...
		$context['topics'][$row['id_topic']] = array(
			...
			'first_post' => array(
				...
				'href' => $scripturl . '?topic=' . $row['id_topic'] . '.msg'. $row['id_first_msg'] . ';topicseen#msg' . $row['id_first_msg'] ,
				'link' => '<a href="' . $scripturl . '?topic=' . $row['id_topic'] . '.msg' . $row['id_first_msg'] . ';topicseen#msg'. $row['id_first_msg'] .'">' . $row['first_subject'] . '</a>'
			),
			'last_post' => array(
				'id' => $row['id_last_msg'],
				'member' => array(
				...
				'href' => $scripturl . '?topic=' . $row['id_topic'] . '.msg' . ($row['num_replies'] == 0 ? $row['id_first_msg'] :  $row['id_last_msg']) . ';topicseen#msg' . $row['id_last_msg'],
				'link' => '<a href="' . $scripturl . '?topic=' . $row['id_topic'] . '.msg' . ($row['num_replies'] == 0 ? $row['id_first_msg'] :  $row['id_last_msg']) . ';topicseen#msg' . $row['id_last_msg'] . '" rel="nofollow">' . $row['last_subject'] . '</a>'
			),
			'new_from' => $row['new_from'],
			'new_href' => $scripturl . '?topic=' . $row['id_topic'] . '.msg' . ($row['num_replies'] == 0 ? $row['id_first_msg'] : $row['id_last_msg']) . ';topicseen#msg' . $row['id_last_msg'], 
			'href' => $scripturl . '?topic=' . $row['id_topic'] . 'msg' . ($row['num_replies'] == 0 ? $row['id_first_msg'] : $row['id_last_msg']) . ';topicseen#msg' . $row['id_last_msg'], 
			'link' => '<a href="' . $scripturl . '?topic=' . $row['id_topic'] .  '.msg' . ($row['num_replies'] == 0 ? $row['id_first_msg'] : $row['id_last_msg']) . ';topicseen#msg' . $row['id_last_msg']. '" rel="nofollow">' . $row['first_subject'] . '</a>',
		...
		if (!empty($modSettings['avatars_on_boardIndex']))
		{
			...
			$context['topics'][$row['id_topic']]['first_post']['member']['avatar'] = set_avatar_data(array(
				...
				'username' => $row['first_poster_name'],
				'id' => $row['id_first_member'],
			));
		}
