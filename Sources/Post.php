...
function Post($post_errors = array())
{
	...
	if ($context['make_event'])
	{
		...
		$context['event']['location'] = isset($_REQUEST['event_location']) ? $pmxcFunc['htmlspecialchars'](stripslashes($_REQUEST['event_location'])) : '';
	...
	$context['require_verification'] = (!$user_info['is_mod'] && !$user_info['is_admin']) && (!empty($modSettings['posts_require_captcha']) && ($user_info['posts'] < $modSettings['posts_require_captcha']) || ($user_info['is_guest'] && $modSettings['guests_require_captcha'] == 1));
	...
	// check verification details, give them some notice.
	if (!empty($context['require_verification']) && getREQcnt('post2') > 0)
	{
		if(!empty($modSettings['need_capcha']) && (($eID = array_search( 'wrong_verification_code', $post_errors, true)) !== false))
		{
			unset($post_errors[$eID]);
			if(isset($_POST['g-recaptcha-response']) && empty($_POST['g-recaptcha-response']))
				$post_errors[] = (isset($_POST['preview']) ? 'recapcha_post_preview' : 'recapcha_post');
		}
		else
...
function Post2()
{
	...
	if (substr_count($_POST['subject'],"http://") > 0 || substr_count($_POST['subject'],"https://") > 0)
		$post_errors[] = 'fatal_subject_link';
...
function AnnouncementSend()
{
	...
	foreach ($rows as $row)
	{
		...
		$cur_language = empty($row['lngfile']) ? $language : $row['lngfile'];
