...
class CreatePost_Notify_Background extends PMX_BackgroundTask
{
	...
	{
		global $pmxcFunc, $sourcedir, $scripturl, $language, $context, $modSettings;
		...
		{
			$quoted_Members = self::getQuotedMembers($msgOptions, $posterOptions);
			$members = array_keys($quoted_Members);
		}
		...
		$request = $pmxcFunc['db_query']('', '
			SELECT mem.id_member, ln.id_topic, ln.id_board, ln.sent, mem.email_address, mem.lngfile, b.member_groups,
				mem.id_group, mem.id_post_group, mem.additional_groups, t.id_member_started, mem.pm_ignore_list,t.id_member_updated 
		...
		// create notifications only for member they have access to the boards
		if (empty($members))
			return true;
		...
		if (!empty($msgOptions['mentioned_members']))
		{
			// Remove mentioned member they can't see the board
			foreach($msgOptions['mentioned_members'] as $mem => $data)
			{
				if(!in_array($mem, $members))
				{
					unset($msgOptions['mentioned_members'][$mem]);
					$done_members[] = $mem;
				}
			}

			// handle mentioned member they can see the board
			if(!empty($msgOptions['mentioned_members']))
		...
		if(!empty($quotedMembers))
		{
			foreach($quotedMembers as $mem => $data)
			{
				if(!in_array($mem, $members))
				{
					unset($quotedMembers[$mem]);
					$done_members[] = $mem;
				}
			}
			if(!empty($quotedMembers))
		...
		foreach ($watched as $member => $data)
		{
			...
			if (!empty($data['pm_ignore_list']) && in_array($data['id_member_updated'], explode(',', $data['pm_ignore_list'])))
				continue;
			elseif (!in_array($type, array('reply', 'topic')) && $notify_types == 2 && $member != $data['id_member_started'])
				continue;
			...
			if ($pref & 0x02)
			{
				$replacements = array(
					...
					'MESSAGE' => trim(un_htmlspecialchars(strip_tags(strtr(parse_bbc($msgOptions['body'], false), array('<br>' => "\n", '</div>' => "\n", '</li>' => "\n", '&#91;' => '[', '&#93;' => ']'))))),
					'UNSUBSCRIBELINK' => $scripturl . '?action=notifyboard;board=' . $topicOptions['board'] . '.0',
				);

				$emaildata = loadEmailTemplate($message_type, $replacements, empty($data['lngfile']) ? $language : $data['lngfile']);
	...
	protected static function handleQuoteNotifications($msgOptions, $posterOptions, $quotedMembers, $prefs, &$done_members, &$alert_rows)
	{
		...
		foreach ($quotedMembers as $id => $member)
		{
			...
			if ($prefs[$id]['msg_quote'] & 0x02)
			{
				...
				$emaildata = loadEmailTemplate('msg_quote', $replacements, empty($member['lngfile']) ? $language : $member['lngfile']);
	...
	protected static function handleMentionedNotifications($msgOptions, $members, $prefs, &$done_members, &$alert_rows)
	{
		...
		foreach ($members as $id => $member)
		{
			...
			if ($prefs[$id]['msg_mention'] & 0x02)
			{
				...
				$emaildata = loadEmailTemplate('msg_mention', $replacements, empty($member['lngfile']) ? $language : $member['lngfile']);
