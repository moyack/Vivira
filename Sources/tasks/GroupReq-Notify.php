...
class GroupReq_Notify_Background extends PMX_BackgroundTask
{
	...
	{
		global $sourcedir, $pmxcFunc, $language, $modSettings, $context, $scripturl;
		...
		if (!empty($moderators))
		{
			...
			if (!empty($data['email']))
			{
				...
				while ($row = $pmxcFunc['db_fetch_assoc']($request))
				{
					...
					$emaildata = loadEmailTemplate('request_membership', $replacements, empty($row['lngfile']) ? $language : $row['lngfile']);
