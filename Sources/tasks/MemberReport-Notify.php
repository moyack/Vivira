...
class MemberReport_Notify_Background extends PMX_BackgroundTask
{
	...
	{
		global $pmxcFunc, $sourcedir, $modSettings, $language, $context, $scripturl;
		...
		if (!empty($notifies['email']))
		{
			...
			foreach ($emails as $this_lang => $recipients)
			{
				...
				$emaildata = loadEmailTemplate('report_member_profile', $replacements, empty($this_lang) ? $language : $this_lang);
