...
function Display()
{
	...
	{
		$date_string = $modSettings['time_format'];
	...
	$context['can_see_likes'] = $context['can_see_likes'] && checkECL_Cookie(true);
	...
	$modSettings['disable_wysiwyg'] = false;
	$context['attached'] = '';
	...
	$context['normal_buttons'] = array(
		...
		'print' => array('test' => 'can_print', 'text' => 'print', 'image' => 'print.png', 'lang' => true, 'custom' => 'rel="nofollow noopener" target="_blank"', 'url' => $scripturl . '?action=printpage;topic=' . $context['current_topic'] . '.0'),
		'notify' => array(
			'lang' => true,
			'test' => 'can_set_notify',
...
function prepareDisplayContext($reset = false)
{
	...
	if (!loadMemberContext($message['id_member'], true))
	{
		...
		$memberContext[$message['id_member']]['href'] = '';
		$memberContext[$message['id_member']]['avatar'] = array(
			'name' => $message['poster_name'],
			'image' => '<img class="avatar default" src="'. $modSettings['avatar_url'] .'/default.png" alt="avatar_'. $message['poster_name'].'" oncontextmenu="return false">',
	...
	$message['body'] = parse_bbc($message['body'], $message['smileys_enabled'], $message['id_msg'], array(), $user_info['is_guest']);
...
function getLikeCount($msgID)
{
	global $pmxcFunc, $pmxCacheFunc;

	if(($Likes = $pmxCacheFunc['get']('Likes-'. $msgID)) === null)
	{
		$Likes = array('count' => 0, 'time' => 0);

		$request = $pmxcFunc['db_query']('', '
			SELECT id_member, like_time
			FROM {db_prefix}user_likes
			WHERE content_id = {int:like_content} AND content_type = {string:like_type}',
			array(
				'like_content' => $msgID,
				'like_type' => 'msg',
			)
		...
		{
			$Likes['count']++;
			$Likes['time'] = $row['like_time'];
		}
		$pmxcFunc['db_free_result']($request);

		$pmxCacheFunc['put']('Likes-'. $msgID, (!empty($Likes['count']) ? $Likes : null) , 21600);
	}
	return $Likes;
}
?>
