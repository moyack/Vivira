...
function CalendarMain()
{
	...
	$calendarOptions = array(
		'start_day' => intval($user_info['is_guest'] ? $txt['dp_firstday'] : (!empty($options['calendar_start_day']) ? $options['calendar_start_day'] : 0)),
	...
	if ($context['calendar_view'] != 'view_list')
		$context['page_title'] .= ' - ' . ($context['calendar_view'] == 'view_week' ? $context['calendar_grid_main']['week_title'] : $txt['months_titles'][$context['current_month']] . ' ' . $context['current_year']);
...
function CalendarPost()
{
	...
	global $modSettings, $topic, $pmxcFunc, $settings, $options;
	...
	if (isset($_POST[$context['session_var']], $_REQUEST['eventid']))
	{
		...
		elseif ($_REQUEST['eventid'] == -1)
		{
			$eventOptions = array(
				...
				'location' => $pmxcFunc['substr']($_REQUEST['event_location'], 0, 255),
				'member' => $user_info['id'],
			);
		...
		if (isset($_POST['start_date']))
		{
			if(isset($_POST['start_time']))
				$d = date_parse($_POST['start_date'] .' '. $_POST['start_time']);
			else
	...
	if ($context['event']['allday'] == true)
	{
		...
		$context['event']['start_time'] = date($js_time_string, time());
		$context['event']['end_time'] = date($js_time_string, time() + 3600);
	}
	// Otherwise, just adjust these to look nice on the input form
	else
	{
		$context['event']['start_time'] = date($js_time_string, strtotime($context['event']['start_iso_gmdate']));
		$context['event']['end_time'] = date($js_time_string, strtotime($context['event']['end_iso_gmdate']));
	}

	// Need this so the user can select a timezone for the event.
	$context['all_timezones'] = pmx_list_timezones($context['event']['start_date']);
	unset($context['all_timezones']['']);

	// If the event's timezone is not in standard list of time zones, prepend it to the list
	...
	loadCSSFile('jquery-ui.datepicker.css', array(), 'pmx_datepicker');
	loadCSSFile('jquery.timepicker.css', array(), 'pmx_timepicker');
...
function iCalDownload()
{
	...
	$filecontents .= 'PRODID:-//PortaMx//PMX ' . (empty($forum_version) ? 0.0 : strtr($forum_version, array('PMX ' => ''))) . '//EN' . "\n";
