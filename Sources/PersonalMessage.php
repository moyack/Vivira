...
function prepareMessageContext($type = 'subject', $reset = false)
{
	...
	if ($context['display_mode'] != 0 && $subjects_request && $type == 'subject')
	{
		...
		if(isset($context['message_labels'][$subject['id_pm']]) && is_array($context['message_labels'][$subject['id_pm']]) && isset($context['labels']) && is_array($context['labels']))
			$FuLab = count($context['message_labels'][$subject['id_pm']]) == count($context['labels']);
		else
			$FuLab = false;
		$output = array(
			'id' => $subject['id_pm'],
			...
			'number_recipients' => is_array($recipients[$subject['id_pm']]['to']) ? count($recipients[$subject['id_pm']]['to']) : 0,
	...
	$isLabeled = false;
	if(isset($context['message_labels'][$message['id_pm']]) && isset($context['labels']))
	{
		if(is_array($context['message_labels'][$message['id_pm']]) && is_array($context['labels']))
			$isLabeled = count($context['message_labels'][$message['id_pm']]) == count($context['labels']);
	}
...
function messagePostError($error_types, $named_recipients, $recipient_ids = array())
{
	...
	foreach ($error_types as $error_type)
	{
		...
		if($error_type == 'wrong_verification_code' && isset($_POST['g-recaptcha-response']) && empty($_POST['g-recaptcha-response']))
			$context['post_error']['messages'][] = (isset($_POST['preview']) || isset($_POST['save_draft']) ? $txt['error_recapcha_post_preview'] : $txt['error_recapcha_post']);

		else if (isset($txt['error_' . $error_type]))
		{
		...
		elseif($error_type == 'wrong_verification_code' && isset($_POST['g-recaptcha-response']) && empty($_POST['g-recaptcha-response']) && isset($_POST['preview']))
			$context['error_type'] = 'serious';
	}
...
function MessageActionsApply()
{
	...
	if (!empty($to_label) && $context['folder'] == 'inbox')
	{
		...
		if ($context['display_mode'] == 2)
		{
			...
			while ($other_pms = $pmxcFunc['db_fetch_assoc']($get_pms))
