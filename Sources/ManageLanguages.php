...
function DownloadLanguage()
{
	...
	foreach ($archive_content as $file)
	{
		...
		$always_copy = 'Themes/default/languages/AvailLangSets.english.php';
		...
		'columns' => array(
			'name' => array(
				...
				'data' => array(
					...
					{
						return '<strong>' . $rowData['name'] . '</strong><br><span class="smalltext attlist_date">' . $txt['languages_download_dest'] . ': ' . $rowData['destination'] . '</span>' . ($rowData['version_compare'] == 'older' ? '<br>' . $txt['languages_download_older'] : '');
					},
			...
			'exists' => array(
				...
				'data' => array(
					'function' => function ($rowData) use ($txt)
					{
						if($rowData['name'] == 'AvailLangSets.english.php')
							return $txt['languages_download_replaced'];
						else
			...
			'copy' => array(
				...
				'data' => array(
					...
					{
						if($rowData['name'] == 'AvailLangSets.english.php')
							return '<input type="hidden" name="copy_file[]" value="' . $rowData['generaldest'] . '"><strong style="font-size:150%;vertical-align:top;">&#10003;</strong>';
						else
...
function list_getLanguages()
{
	global $settings, $pmxcFunc, $pmxCacheFunc, $language, $context, $txt;
	...
	foreach ($context['languages'] as $lang)
	{
		...
		require($settings['default_theme_dir'] . '/languages/LangSettings.' . $lang['filename'] . '.php');
...
function ModifyLanguageSettings($return_config = false)
{
	...
	$config_vars = array(
		...
		'errorlang' => array('errorlang', $txt['default_errloglang'], 'file', 'select', array(), 'errorLanguage', 'disabled' => $settings_not_writable),
	);
	...
	foreach ($context['languages'] as $lang)
	{
		...
		$config_vars['errorlang'][4][$lang['filename']] = array($lang['filename'], strtr($lang['name'], array('-utf8' => ' (UTF-8)')));
	}
...
{
	global $settings, $context, $pmxcFunc, $pmxCacheFunc, $txt, $modSettings, $boarddir, $sourcedir, $language;
	...
	loadLanguage('LangSettings', $context['lang_id'], false, true);
	...
	if (!empty($_POST['delete_main']) && $context['lang_id'] != 'english')
	{
		...
		if (file_exists($curPath . '/agreement.' . $context['lang_id'] . '.php'))
					unlink($curPath . '/agreement.' . $context['lang_id'] . '.php');
	...
	if (!empty($_POST['save_main']) && !$current_file)
	{
		...
		$current_data = implode('', file($settings['default_theme_dir'] . '/languages/LangSettings.' . $context['lang_id'] . '.php'));
		...
		$fp = fopen($settings['default_theme_dir'] . '/languages/LangSettings.' . $context['lang_id'] . '.php', 'w+');
	...
	// Quickly load LangSetting language entries.
	$old_txt = $txt;
	$old_modSettings = $modSettings;

	require($settings['default_theme_dir'] . '/languages/LangSettings.' . $context['lang_id'] . '.php');
	$context['lang_file_not_writable_message'] = is_writable($settings['default_theme_dir'] . '/languages/LangSettings.' . $context['lang_id'] . '.php') ? '' : sprintf($txt['lang_file_not_writable'], $settings['default_theme_dir'] . '/languages/LangSettings.' . $context['lang_id'] . '.php');
