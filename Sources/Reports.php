...
function ReportsMain()
{
	...
	$context['report_buttons'] = array(
		'generate_reports' => array('text' => 'generate_reports', 'image' => 'print.png', 'lang' => true, 'url' => $scripturl . '?action=admin;area=reports;'. $context['session_var'] . '=' . $context['session_id'] . ';', 'active' => true),
		'print' => array('text' => 'print', 'image' => 'print.png', 'lang' => true, 'url' => $scripturl . '?action=admin;area=reports;rt=' . $context['report_type']. ';st=print', 'custom' => 'target="_blank" rel="noopener"'),
	);
...
function MemberGroupsReport()
{
	...
	foreach ($rows as $row)
	{
		...
		$group = array(
			...
			'min_posts' => $row['min_posts'] == -1 ? $txt['report_na'] : $row['min_posts'],
...
//	loadLanguage('ManagePermissions');
	...
	// Certain permissions should not really be shown.
	$disabled_permissions = array();
	$disabled_permissions[] = 'profile_website_own';
	$disabled_permissions[] = 'profile_remove_any';
	$disabled_permissions[] = 'profile_identity_own';
	$disabled_permissions[] = 'profile_other_own';
	$disabled_permissions[] = 'profile_title_own';
