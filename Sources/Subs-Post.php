...
function sendmail($to, $subject, $message, $from = null, $message_id = null, $send_html = false, $priority = 3, $hotmail_fix = null, $is_private = false)
{
	...
	foreach($to_array as $k => $v)
	{
		...
		{
			log_error(sprintf($txt['mail_send_unable'], $v));
			unset($to_array[$k]);
		}
	...
	// call SEF..
	pmxsef_EmailOutput($subject, $message, $headers);
...
function sendpm($recipients, $subject, $message, $store_outbox = false, $from = null, $pm_head = 0)
{
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		...
		{
			$notifications[empty($row['lngfile']) ? $language : $row['lngfile']][] = $row['email_address'];
		}
 ...
 */
function smtp_mail($mail_to_array, $subject, $message, $headers, $fromTask = false)
{
	global $modSettings, $webmaster_email, $txt;

	$modSettings['smtp_mail_error'] = '-1';
	$modSettings['fromTask'] = $fromTask;
	...
	foreach ($mail_to_array as $i => $mail_to)
	{
		...
		$modSettings['current_mail_to'] = $mail_to;
...
function server_parse($message, $socket, $response)
{
	...
	while (substr($server_response, 3, 1) != ' ')
		if (!($server_response = fgets($socket, 256)))
		{
			...
			log_error($txt['smtp_bad_response'] .' - eMail: '. $modSettings['current_mail_to']);
			$modSettings['smtp_mail_error'] = trim(substr($server_response, 0, 3));
			return false;
		}
	...
	{
		if(!$modSettings['fromTask'])
			log_error($txt['smtp_error'] . $server_response .' - eMail: '. $modSettings['current_mail_to']);
		$modSettings['smtp_mail_error'] = trim(substr($server_response, 0, 3));
		return false;
	}
