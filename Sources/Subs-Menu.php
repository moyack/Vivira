			...
			if ((!isset($area['enabled']) || $area['enabled'] != false) && (empty($area['permission']) || allowedTo($area['permission'])))
			{
				...
				if (isset($area['label']) || (isset($txt[$area_id]) && !isset($area['select'])))
				{
					...
					if (empty($area['hidden']))
					{
						...
						elseif (isset($area['icon']) && file_exists($settings['default_theme_dir'] . '/images/icons/' . $area['icon']))
							$menu_context['sections'][$section_id]['areas'][$area_id]['icon'] = '<img src="' . $settings['default_images_url'] . '/images/icons/' . $area['icon'] . '" alt="">';
						elseif (isset($area['icon']))
							$menu_context['sections'][$section_id]['areas'][$area_id]['icon'] = '<span class="generic_icons ' . $area['icon'] . '"></span>';
						else
						{
							$tmp = array_keys($menu_context['sections'][$section_id]['areas']);
							if(isset($tmp[0]) && preg_match('/label[0-9]+/', $tmp[0]) == 0)
								$menu_context['sections'][$section_id]['areas'][$area_id]['icon'] = '<span class="generic_icons ' . $area_id . '"></span>';
							else
								$menu_context['sections'][$section_id]['areas'][$area_id]['icon'] = '<span class="generic_icons label"></span>';
						}
