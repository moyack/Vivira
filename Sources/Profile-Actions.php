				...
				'data' => array(
					'function' => function ($warning) use ($scripturl, $txt)
					{
						...
						<div class="floatright">
							<a href="' . $scripturl . '?action=moderate;area=notice;nid=' . $warning['id_notice'] . '" onclick="window.open(this.href, \'\', \'scrollbars=yes,resizable=yes,width=400,height=250\');return false;" target="_blank" rel="noopener" class="new_win" title="' . $txt['profile_warning_previous_notice'] . '"><span class="generic_icons filter centericon"></span></a>
						</div>';
...
/**
 * GDPR Function to export User Profile and Posts.
 */
function GDPR_ExportData()
{
	global $txt, $user_info, $pmxcFunc;

	// make secure .. only the user itself or admins can do this !!
	if(!$user_info['is_admin'])
	{
		if(intval($_REQUEST['u']) != $user_info['id'])
			fatal_error($txt['gdrp_export_fail'], false);
	}

	if(!isset($_REQUEST['sa']))
		redirectexit('action=profile;u='. $_REQUEST['u'] .';area=summary');
	else
		$subFunc = $_REQUEST['sa'];

	if($subFunc == 'profile')
	{
		$orgLang = $user_info['language'];
		loadLanguage('Profile', 'english, true, true');
	
		$request = $pmxcFunc['db_query']('', '
			SELECT id_member, member_name, real_name, email_address, date_registered, posts, gender, location, personal_text, usertitle, birthdate, website_title, website_url, signature
			FROM {db_prefix}members
			WHERE id_member = {int:selected_member}',
			array(
				'selected_member' => intval($_REQUEST['u']),
			)
		);
		...
		// kopfzeile
		$data = $txt['exp_uid'] .','. $txt['exp_username'] .','. $txt['exp_realname'] .','. $txt['exp_email']  .','. $txt['exp_date_register'] .','. $txt['exp_posts'] .','. $txt['exp_gender'];
		$data .= ','. $txt['exp_location'] .','. $txt['exp_pers_text'].','. $txt['exp_user_ttl'] .','. $txt['exp_birstdate'] .','. $txt['exp_website_ttl'] .','. $txt['exp_wbsite_url'] .','. $txt['exp_signature'] ."\r\n";

		$data .= '"' . $row['id_member'] . '",';
		$data .= '"' . $row['member_name'] . '",';
		$data .= '"' . FormatCSVData($row['real_name']) . '",';
		$data .= '"' . $row['email_address'] . '",';
		$data .= '"' . date("F j, Y, g:i a",$row['date_registered']) . '",';
		$data .= '"' . $row['posts'] . '",';
		$data .= '"' . $txt['gender_type'][$row['gender']] . '",';
		$data .= '"' . $row['location'] . '",';
		$data .= '"' . FormatCSVData($row['personal_text']) . '",';
		$data .= '"' . FormatCSVData($row['usertitle']) . '",';
		$data .= '"' . $row['birthdate'] . '",';
		$data .= '"' . FormatCSVData($row['website_title']) . '",';
		$data .= '"' . $row['website_url'] . '",';
		$data .= '"' . FormatCSVData($row['signature']) . '",';
		$data .= "\r\n";

		loadLanguage('Profile', $orgLang, true, true);
		SendData($data, 'ProfileData.csv');
	}

	// export Posts
	else if($subFunc == 'posts')
	{
		$orgLang = $user_info['language'];
		loadLanguage('Profile', 'english, true, true');
		...
		$data = '';
		$request = $pmxcFunc['db_query']('', '
			SELECT poster_time,subject,body
		...
		while ($row = $pmxcFunc['db_fetch_assoc']($request))
		{
			$data .= $txt['exp_post_subject'] . $row['subject'] . "\r\n";
			$data .= $txt['exp_post_date'] . date("F j, Y, g:i a", $row['poster_time']) . "\r\n";
			$row['body'] = preg_replace('/\[quote[^\]]*\]/', '[quote]', $row['body']);
			$row['body'] = preg_replace('/\[member\=\d+\]*[^\[]*\[\/member\]/', '@USERNAME', $row['body']);
			$data .= $txt['exp_post_body'] . $row['body'] . "\r\n\r\n";
		}
		$pmxcFunc['db_free_result']($request);

		loadLanguage('Profile', $orgLang, true, true);
		SendData($data, 'PostData.txt');
	}
}

/**
/* CSV converting for Userdata
**/
