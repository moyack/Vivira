	...
	if (!$pre_query || !empty($topic_ids))
	{
		...
		$result = $pmxcFunc['db_query']('substring', '
			...
			FROM {db_prefix}topics AS t
				...
				LEFT JOIN {db_prefix}members AS meml ON (meml.id_member = ml.id_member)
				LEFT JOIN {db_prefix}members AS memf ON (memf.id_member = mf.id_member)' . (!empty($modSettings['avatars_on_boardIndex']) ? '
		...
		while ($row = $pmxcFunc['db_fetch_assoc']($result))
		{
			...
			$context['topics'][$row['id_topic']] = array_merge($row, array(
				...
				'first_post' => array(
					...
					'href' => $scripturl . '?topic=' . $row['id_topic'] . '.msg'. $row['id_first_msg'] .'#'. $row['id_first_msg'],
					'link' => '<a href="' . $scripturl . '?topic=' . $row['id_topic'] . '.msg'. $row['id_first_msg'] .'#msg'. $row['id_first_msg'] .'">' . $row['first_subject'] . '</a>',
				),
				'last_post' => array(
					'id' => $row['id_last_msg'],
					'member' => array(
					...
					'href' => $scripturl . '?topic=' . $row['id_topic'] . ($user_info['is_guest'] ? ('.' . (!empty($options['view_newest_first']) ? 0 : ((int) (($row['num_replies']) / $context['pageindex_multiplier'])) * $context['pageindex_multiplier']) . '#msg' . $row['id_last_msg']) : (($row['num_replies'] == 0 ? '.0' : '.msg' . $row['id_last_msg']) . '#msg'. $row['id_last_msg'])),
					'link' => '<a href="' . $scripturl . '?topic=' . $row['id_topic'] . ($user_info['is_guest'] ? ('.' . (!empty($options['view_newest_first']) ? 0 : ((int) (($row['num_replies']) / $context['pageindex_multiplier'])) * $context['pageindex_multiplier']) . '#msg' . $row['id_last_msg']) : (($row['num_replies'] == 0 ? '.0' : '.msg' . $row['id_last_msg']) . '#msg'. $row['id_last_msg']) . '" ' . ($row['num_replies'] == 0 ? '' : 'rel="nofollow"')) . '>' . $row['last_subject'] . '</a>'
				),
				...
				'new_from' => $row['id_msg_modified'],
				'newtime' => $row['id_msg_modified'],
				'new_href' => $scripturl . '?topic=' . $row['id_topic'] .'.msg'. ($row['new_from'] > $row['id_first_msg'] ? $row['new_from'] .'#msg'.$row['new_from'] : ($row['id_msg_modified'] > $row['id_last_msg'] ? $row['id_last_msg'] .'#msg'. $row['id_last_msg'] : $row['id_msg_modified'] .'#msg'. $row['id_msg_modified'])),
				'pages' => $pages,
...
function QuickModeration()
{
    ...
    if(isset($_SESSION) && isset($_SESSION['came_from']))
    {
        $redirect_url = $_SESSION['came_from'];
