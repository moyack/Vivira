...
function resetPassword($memID, $username = null)
{
	...
	$emaildata = loadEmailTemplate('change_password', $replacements, empty($lngfile) ? $language : $lngfile);
...
{
	global $sourcedir, $txt, $pmxcFunc, $user_info, $modSettings;

	$errors = array();
	$actNames = explode(',', $modSettings['sef_actions']);

	// Don't use too long a name.
	if ($pmxcFunc['strlen']($username) > 25)
		$errors[] = array('lang', 'error_long_name');

	elseif (strlen(preg_replace('~[\_\-]+~', '', $username)) < 4)
		$errors[] = array('lang', 'error_username_unavailable');

	elseif(ltrim($username, ' _-1234567890') !== $username)
		$errors[] = array('lang', 'error_username_unavailable');

	elseif(rtrim($username, ' _-1234567890') !== $username)
		$errors[] = array('lang', 'error_username_unavailable');

	elseif(in_array(strtolower($username), $actNames) || in_array(strtolower($username), array('forum', 'board', 'topic', 'msg', 'page', 'block', 'cat', 'art', 'categorie', 'article', 'portal')))
		$errors[] = array('lang', 'error_username_reserved');

	// Only these characters are permitted.
	elseif(preg_replace('~[a-zA-Z0-9\-\_]+~', '', $username) !== '' || strpos($username, '[code') !== false || strpos($username, '[/code') !== false)
		$errors[] = array('lang', 'invalid_characters_username');

	if(stripos($username, $txt['guest_title']) !== false || stripos($username, 'portamx') !== false || stripos($username, 'spider') !== false)
		$errors[] = array('done', '(' . $pmxcFunc['htmlspecialchars']($username) . ') ' . $txt['name_reserved']);
	...
	fatal_error($message, empty($error[2]) || $user_info['is_admin'] || (isset($modSettings['write_tolog']) && empty($modSettings['write_tolog'])) ? false : $error[2]);
}
