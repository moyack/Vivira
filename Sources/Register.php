...
/**
 * Show the GDPR document
 */
function ShowGDPR()
{
	global $context, $modSettings, $settings, $user_info, $txt, $language, $pmxcFunc;

	if(getREQcnt('help,rules') == 2)
		redirectexit();

	loadTemplate('Register');
	$_SESSION['language'] = $user_info['language'];
	loadLanguage('LangSettings+index+Login', $user_info['language'], true, true);
	...
	if (file_exists($settings['default_theme_dir'] .'/languages/'. $regfile .'.php'))
	{
		...
		if(!empty($modSettings['gdpr_enabled']))
		{
			...
			if(isset($user_info['gdpr_date']) && !empty($user_info['gdpr_date']) && strtotime($modSettings['gdpr_last_update']) > $user_info['gdpr_date'])
	...
	if(!empty($modSettings['gdpr_enabled']))
		$context['sub_template'] = 'accept_gdpr';
	$context['page_title'] = (!empty($modSettings['gdpr_enabled']) ? $txt['gdpr_registration_agreement'] : $txt['registration_agreement']);
}
...
{
	global $txt, $boarddir, $context, $modSettings, $user_info, $settings;
	...
	if ($current_step == 1 && (isset($_POST['accept_agreement']) || isset($_POST['accept_agreement_coppa'])))
	{
		...
		if ($context['show_coppa'])
		{
			...
			{
				loadLanguage('Login+LangSettings');
	...
	if ($context['require_agreement'])
	{
		...
		if(!empty($modSettings['gdpr_enabled']))
		{
			$fileName = $settings['default_theme_dir'] . '/languages/gdpr_agreement.' . $user_info['language'] . '.php';
			if (file_exists($fileName))
			{
				loadLanguage('Login', $user_info['language']);
				$context['agreement'] = preg_replace('/<\?php[^>]*>/', '', file_get_contents($fileName));
				$context['agreement'] = parse_bbc(str_replace(array('{name}', '{street}', '{location}', '{country}', '{email}', '{hoster}', '{age}'), array($modSettings['gdpr_owner_name'], $modSettings['gdpr_owner_street'], $modSettings['gdpr_owner_location'], $modSettings['gdpr_owner_country'], $modSettings['gdpr_owner_email'], $modSettings['gdpr_owner_hoster'], $modSettings['coppaAge']), $context['agreement']));
				$context['agreement'] .= $txt['gdpr_agreement_footer'];
				if(isset($user_info['gdpr_date']) && !empty($user_info['gdpr_date']) && strtotime($modSettings['gdpr_last_update']) > $user_info['gdpr_date'])
					$context['agreement'] .= str_replace('{date}', date_format(date_create_from_format('Y-m-d', $modSettings['gdpr_last_update']), $txt['gdpr_to_format']), $txt['gdpr_agreement_updated']);
			}
		...
		{
			$fileName = 'agreement.' . $user_info['language'] . '.php';
			if (file_exists($settings['default_theme_dir'] . '/languages/'. $fileName))
			{
				$context['agreement'] .= preg_replace('/<\?php[^>]*>/', '', file_get_contents($settings['default_theme_dir'] . '/languages/'. $fileName));
				$context['agreement'] = parse_bbc($context['agreement']);
			}
		}
	}
...
function Register2()
{
	...
	if (!empty($modSettings['reg_verification']))
	{
		...
		if (is_array($context['visual_verification']))
		{
			...
			{
				if($error == 'wrong_verification_code' && !empty($modSettings['need_capcha']) && isset($_POST['g-recaptcha-response']) && empty($_POST['g-recaptcha-response']))
					$reg_errors[] = $txt['error_recapcha_post'];
				else
	...
	// setup default smiley set
	$_POST['smiley_set'] = !empty($modSettings['smiley_sets_default']) ? $modSettings['smiley_sets_default'] : 'default';
	...
	$regOptions = array(
		...
		'theme_vars' => array(),
		'smiley_set' => !empty($_POST['smiley_set']) ? $_POST['smiley_set'] : '',
...
function Activate()
{
	...
	if (!empty($_REQUEST['sa']) && $_REQUEST['sa'] == 'resend' && ($row['is_activated'] == 0 || $row['is_activated'] == 2) && (!isset($_REQUEST['code']) || $_REQUEST['code'] == ''))
	{
		...
		$emaildata = loadEmailTemplate('resend_activate_message', $replacements, empty($row['lngfile']) ? $language : $row['lngfile']);
