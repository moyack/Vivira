...
function create_control_richedit($editorOptions)
{
	...
	if (empty($context['controls']['richedit']))
	{
		...
		loadCSSFile('jquery.sceditor.css', array('force_current' => false, 'validate' => true), 'pmx_sceditor');
		...
		loadJavascriptFile('jquery.sceditor.pmx.min.js', array(), 'pmx_sceditor');
		addInlineJavascript('
	var pmx_edit_mode = true;
	var pmx_smileys_url = \'' . $settings['smileys_url'] . '\';
	var bbc_quote_from = \'' . addcslashes($txt['quote_from'], "'") . '\';
	var bbc_quote = \'' . addcslashes($txt['quote'], "'") . '\';
	var popup_close = \'' . addcslashes($txt['find_close'], "'") . '\';
	var bbc_search_on = \'' . addcslashes($txt['quote_time'], "'") . '\';');
	...
	$context['controls']['richedit'][$editorOptions['id']] = array(
		...
		'locale' => isset($txt['lang_locale']) ? $txt['lang_locale'] : 'en_US',
	...
	if($isAction)
	{
		if(in_array($isAction, array('post', 'post2')) || ($isAction == 'calendar'	&& isset($_GET['sa']) && $_GET['sa'] == 'post'))
		{
			...
			if(!isset($cururl['action']) || (isset($cururl['action']) && !in_array($cururl['action'], array('post', 'post2')) && (!isset($cururl['sa']) || (isset($cururl['sa']) && $cururl['sa'] !== 'post')) && !isset($cururl['id_draft'])))
			{
				...
				$context['controls']['richedit'][$editorOptions['id']]['cancel_link'] = (isset($_SESSION['cancel_link']) ? $_SESSION['cancel_link'] : (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : $_SERVER['REQUEST_URL']));
		}
	...
	if (empty($context['bbc_tags']))
	{
		...
		*/
		...
		$context['bbc_tags'][] = array(
			...
			array(
				'code' => 'youtube',
				'description' => $editortxt['insert_a_youtube_video']
			),
		);
...
function create_control_verification(&$verificationOptions, $do_test = false)
{
	...
	global $context, $user_info, $scripturl, $language, $sourcedir;
	...
	if ($isNew)
		$context['controls']['verification'][$verificationOptions['id']] = array(
			...
			'text_value' => '',
			'questions' => array(),
			'can_recaptcha' => !empty($modSettings['recaptcha_enabled']) && !empty($modSettings['recaptcha_public_key']) && !empty($modSettings['recaptcha_private_key']),
		);
	...
	if ($thisVerification['can_recaptcha'])
	{
		...
		$thisVerification['recaptcha_private_key'] = preg_replace('/(0-9a-zA-Z_){40}/', '$1', $modSettings['recaptcha_private_key']);
	...
	if (!empty($thisVerification['number_questions']) && empty($modSettings['question_id_cache']))
	{
		if (($modSettings['question_id_cache'] = $pmxCacheFunc['get']('verificationQuestions')) == null)
		{
			...
			while ($row = $pmxcFunc['db_fetch_assoc']($request))
			{
				...
				$row['lngfile'] = !empty($row['lngfile']) ? $row['lngfile'] : $language;
	...
	if ($do_test)
	{
		...
		if ($thisVerification['can_recaptcha'])
		{
			$reCaptcha = new \ReCaptcha\ReCaptcha($modSettings['recaptcha_public_key']);
			// Is there a reCAPTCHA response?
...
function AutoSuggest_Search_PMXVersions()
{
	...
	if (empty($versions))
		$versions = array('PortaMx-Forum 1.4');
