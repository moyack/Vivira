...
$GLOBALS['search_versions'] = array(
	...
	'forum_version' => 'PortaMx-Forum 1.0',
...
function PlushSearch2()
{
	...
	if (($user_info['is_guest'] && !empty($modSettings['need_capcha'])) || ($user_info['is_guest'] && !empty($modSettings['search_enable_captcha']) && empty($_SESSION['ss_vv_passed']) && (empty($_SESSION['last_ss']) || $_SESSION['last_ss'] != $search_params['search'])))
	{
		// If we come from another search box tone down the error...
		if (!isset($_REQUEST['search_vv']) && empty($modSettings['recaptcha_enabled']) && empty($_SESSION['ss_vv_passed']))
			$context['search_errors']['need_verification_code'] = true;
		else if(!empty($_SESSION['last_ss']) && $_SESSION['last_ss'] != $search_params['search'])
		{
			require_once($sourcedir . '/Subs-Editor.php');
			$verificationOptions = array(
				'id' => 'search',
			...
			if (is_array($context['require_verification']))
			{
				...
				{
					if(strpos($error, 'verification') !== false)
					{
						if(!empty($modSettings['need_capcha']))
							$context['search_errors']['recapcha_post'] = true;
						else
