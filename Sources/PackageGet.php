...
function PackageServers()
{
	...
	$(\'.download_new_package\').on(\'click\', function() {
		var collapseState = $(\'#add_server\').css(\'display\');
		var icon = $(\'.download_new_package\').children(\'span\');
		var collapsedElm = $(\'#add_server\');

		if (collapseState == \'none\')
		{
			collapsedElm.show(400);
			icon.removeClass(\'toggle_down\').addClass(\'toggle_up\');
			icon.prop(\'title\', '. JavaScriptEscape($txt['hide']) .');
		}
		else
		{
			collapsedElm.hide(400);
			icon.removeClass(\'toggle_up\').addClass(\'toggle_down\');
			icon.prop(\'title\', '. JavaScriptEscape($txt['show']) .');
		}
	});

	$(\'.download_by_url\').on(\'click\', function() {
		var collapseState = $(\'#download_url\').css(\'display\');
		var icon = $(\'.download_by_url\').children(\'span\');
		var collapsedElm = $(\'#download_url\');
...
function PackageDownload()
{
	...
	if (preg_match('~^http://docserver\.portamx\.com/~', $_REQUEST['package']) == 1 && strpos($_REQUEST['package'], 'dlattach') === false && isset($_REQUEST['auto']))
