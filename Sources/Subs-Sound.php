...
function createWaveFile($word)
{
	...
	$uiIp = str_replace(array('.', ':'), '', $user_info['ip']);
	$uiIp2 = str_replace(array('.', ':'), '', $user_info['ip2']);
	if (($ip = $pmxCacheFunc['get']('wave_file-' . $uiIp)) > 2 || ($ip2 = $pmxCacheFunc['get']('wave_file-' . $uiIp2)) > 2)
		die(header('HTTP/1.1 400 Bad Request'));
	$pmxCacheFunc['put']('wave_file-' . $uiIp, $ip ? $ip + 1 : 1, 20);
	$pmxCacheFunc['put']('wave_file-' . $uiIp2, $ip2 ? $ip2 + 1 : 1, 20);
