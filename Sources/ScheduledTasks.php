...
function AutoTask()
{
	global $time_start, $pmxcFunc, $modSettings, $context;
...
{
	global $scripturl, $txt, $sourcedir, $language, $context, $pmxcFunc;

	if(!defined('is_sheduled_task'))
		define('is_sheduled_task', true);

	// Grab all the items awaiting approval anoard - clear up any things that are no longer relevant.
	$request = $pmxcFunc['db_query']('', '
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		...
		$members[$row['id_member']] = array(
			...
			'language' => !empty($row['lngfile']) ? $row['lngfile'] : $language,
...
{
	global $pmxcFunc, $pmxCacheFunc, $modSettings, $context, $sourcedir, $boarddir, $db_type;
	...
	// Delete LangDetect Files older 7 days
	if(!empty($modSettings['langdetect_log']))
	{
		$maxDays = 15;
		while ($maxDays > 6)
		{
			$fName = $boarddir .'/LangDetect/data-'. date('Y-m-d', strtotime('-'. $maxDays .' day')) .'.log';
...
{
	global $is_weekly, $txt, $mbname, $scripturl, $sourcedir, $pmxcFunc, $pmxCacheFunc, $context, $modSettings, $language;
	...
	foreach ($langs as $lang)
	{
		...
		loadLanguage('LangSettings', $lang);
		...
		$langtxt[$lang] = array(
			...
			'lang_locale' => $txt['lang_locale'],
			'lang_character_set' => $txt['lang_character_set'],
			'lang_rtl' => $txt['lang_rtl'],
			'number_format' => $txt['number_format'],
			'numforms' => $txt['numforms'],
			'default_time_format' => $txt['default_time_format']
		);
	}
	...
	{
		$mLng = !empty($member['lang']) ? $member['lang'] : $language;
		...
		// Right character set!
		$context['character_set'] = empty($modSettings['global_character_set']) ? $langtxt[$mLng]['char_set'] : $modSettings['global_character_set'];

		// Do the start stuff!
 		$email = array(
			'subject' => $mbname . ' - ' . $langtxt[$mLng]['subject'],
			...
			foreach ($types['topic'] as $id => $board)
				foreach ($board['lines'] as $topic)
					if (in_array($mid, $topic['members']))
					{
						if (!$titled)
						{
							$email['body'] .= "\n" . $langtxt[$mLng]['new_topics'] . ':' . "\n" . '-----------------------------------------------';
							$titled = true;
						}
						$email['body'] .= "\n" . sprintf($langtxt[$mLng]['topic_lines'], $topic['subject'], $board['name']);
					}
			if ($titled)
				$email['body'] .= "\n";
		}
			...
			foreach ($types['reply'] as $id => $board)
				foreach ($board['lines'] as $topic)
					if (in_array($mid, $topic['members']))
					{
						if (!$titled)
						{
							$email['body'] .= "\n" . $langtxt[$mLng]['new_replies'] . ':' . "\n" . '-----------------------------------------------';
							$titled = true;
						}
						$email['body'] .= "\n" . ($topic['count'] == 1 ? sprintf($langtxt[$mLng]['replies_one'], $topic['subject']) : sprintf($langtxt[$mLng]['replies_many'], $topic['count'], $topic['subject']));
					}
				...
				foreach ($type as $id => $board)
					foreach ($board['lines'] as $topic)
						if (in_array($mid, $topic['members']))
						{
							if (!$titled)
							{
								$email['body'] .= "\n" . $langtxt[$mLng]['mod_actions'] . ':' . "\n" . '-----------------------------------------------';
								$titled = true;
							}
							$email['body'] .= "\n" . sprintf($langtxt[$mLng][$note_type], $topic['subject']);
						}
			}
		}
		if ($titled)
...
{
	global $modSettings, $pmxcFunc, $sourcedir, $context, $txt;
	...
	// fiter out bad to adresses
	foreach ($emails as $id => $email)
	{
		// Validate e-mail
		if (filter_var($email['to'], FILTER_VALIDATE_EMAIL) === false)
		{
	...
	foreach ($emails as $id => $email)
	{
		$modSettings['smtp_mail_error'] = '0';

		// call SEF..
		pmxsef_EmailOutput($email['subject'], $email['body'], $email['headers']);
		...
		if (empty($modSettings['mail_type']) || $modSettings['smtp_host'] == '')
		{
			...
			$result = smtp_mail(array($email['to']), $email['subject'], $email['body'], $email['headers'], true);

		// Hopefully it sent?
		if (!$result)
		{
			if(intval($modSettings['smtp_mail_error']) < 400)
...
{
	global $sourcedir, $txt, $language, $errorlang, $forum_version, $modSettings, $pmxcFunc, $context;
	...
	$js_files = array();
	$lang = empty($errorlang) ? 'english' : $errorlang;
	loadLanguage('LangSettings', $lang, false);
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		$js_files[$row['id_file']] = array(
			...
			'path' => $row['path'],
			'parameters' => sprintf($row['parameters'], $lang, urlencode($txt['default_time_format']), urlencode($forum_version)),
		);
	}
	...
	loadEssentialThemeData();
	loadLanguage('Errors', $lang, false);
...
{
	global $sourcedir, $scripturl, $pmxcFunc, $modSettings, $context, $language;
	...
	foreach ($members as $row)
	{
		...
		$emaildata = loadEmailTemplate('paid_subscription_reminder', $replacements, empty($row['lngfile']) ? $language : $row['lngfile']);
...
{
	global $pmxcFunc, $sourcedir, $context;
...
{
	global $pmxcFunc, $sourcedir, $context, $modSettings;
