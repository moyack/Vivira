...
<div class="smalltext" style="text-align: left; margin: 1ex;">
	...
	if ($_SESSION['view_queries'] == 1 && !empty($db_cache))
		foreach ($db_cache as $q => $qq)
		{
			...
			echo '
	<strong>', $is_select ? '<a href="' . $scripturl . '?action=viewquery;qq=' . ($q + 1) . '#qq' . $q . '" target="_blank" rel="noopener" class="new_win" style="text-decoration: none;">' : '', nl2br(str_replace("\t", '&nbsp;&nbsp;&nbsp;', $pmxcFunc['htmlspecialchars'](ltrim($qq['q'], "\n\r")))) . ($is_select ? '</a></strong>' : '</strong>') . '<br>
	&nbsp;&nbsp;&nbsp;';
...
{
	global $modSettings, $user_info, $user_profile, $pmxcFunc, $sourcedir;
	...
	foreach ($logs as $log)
	{
		...
		else if(isset($log['extra']['group']))
		{
			if(isset($log['extra']['member']))
				$memID = $log['extra']['member'];
			else
				$memID = $user_info['id'];
		}
		if(!isset($user_info['ip']))
		{
			loadMemberData(array($memID), false, 'minimal');
			$user_info['ip'] = $user_profile['ip'];
		}
		$inserts[] = array(
