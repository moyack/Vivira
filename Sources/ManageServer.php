...
{
	global $context, $txt, $scripturl, $cache_enable, $boarddir, $pmxCacheFunc;

	$context['settings_message'] = $txt['sef_settings_description'];
	$canEnabled = file_exists($boarddir .'/.htaccess') && !empty($cache_enable);
	$config_vars = array(
		array('sef_enabled', $txt['sef_enabled'], 'db', 'check', null, 'sef_enabled', 'disabled' => empty($canEnabled)),
...
{
	global $context, $scripturl, $db_type, $txt, $pmxCacheFunc;
	...
	if(($db_type) && $db_type = 'msqli')
		$detected['dbcache'] = $txt['dbcache_cache'];
...
{
	global $txt, $helptxt, $context, $modSettings, $sourcedir, $user_info, $pmxcFunc;
			...
			if (isset($modSettings[$config_var[1]]))
			{
				switch ($config_var[0])
				{
					...
					default:
						...
						if($config_var[1] == 'gdpr_last_update')
						{
							if(empty($value))
								$value = '2018-05-26';
							$value = date_format(date_create_from_format('Y-m-d', $value), $txt['gdpr_to_format']);
						}
				}
			}
			else
...
function saveSettings(&$config_vars)
{
	...
	$config_strs = array(
		'mtitle', 'mmessage',
		'language', 'errorlang', 'mbname', 'boardurl',
		'cookiename',
		'webmaster_email',
...
{
	global $sourcedir, $pmxcFunc, $pmxCacheFunc, $txt;
	...
	foreach ($config_vars as $var)
	{
		...
		{
			if($var[1] == 'gdpr_last_update')
			{
				if(!empty($_POST[$var[1]]))
					$setArray[$var[1]] = date_format(date_create_from_format($txt['gdpr_to_format'], $_POST[$var[1]]), 'Y-m-d');
				else
					$setArray[$var[1]] = '';
			}
			else
