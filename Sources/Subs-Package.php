...
function parsePackageInfo(&$packageXML, $testing_only = true, $method = 'install', $previous_version = '')
{
	...
	$script = false;
	$the_version = 'pmx'. strtr($forum_version, array('PortaMx-Forum' => ''));
...
function compareVersions($version1, $version2)
{
	...
	foreach (array(1 => $version1, $version2) as $id => $version)
	{
		...
		if(!empty($parts))
		{
			...
			$versions[$id] = array(
				'devel' => empty($parts[2]) ? '' : $parts[2],
				'major' => isset($parts[3]) ? (int) $parts[3] : 0,
				'minor' => isset($parts[4]) ? (int) $parts[4] : (isset($parts[5]) && $parts[5] == '*' ? (isset($versions[1]['minor']) ? $versions[1]['minor'] : 0) : 0),
				'type' => isset($parts[5]) && $parts[5] == '*' ? (isset($versions[1]['type']) ? $versions[1]['type'] : '') : $parts[5],
				'type_major' => !isset($parts[6]) ? (isset($parts[5]) && $parts[5] == '*' ? (isset($versions[1]['type_major']) ? $versions[1]['type_major'] : 0) : (int) $parts[6]) : 0,
				'type_minor' => !isset($parts[7]) ? (isset($parts[5]) && $parts[5] == '*' ? (isset($versions[1]['type_minor']) ? $versions[1]['type_minor'] : 0) : (int) $parts[7]) : 0,
				'dev' => isset($parts[8]) && isset($parts[9]) ? (int) $parts[9] : 0,
			);
		}
		else
			return -1;
...
function package_create_backup($id = 'backup')
{
	...
	$base_files = array('index.php', 'SSI.php', 'cron.php', 'ssi_examples.php', 'ssi_examples.shtml', 'subscriptions.php');
