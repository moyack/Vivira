...
function ModifyProfile($post_errors = array())
{
	...
	if(empty($context['user']['is_owner']) && !empty($modSettings['gdpr_enabled']) && show_gdpr_agreement())
		redirectexit();
	...
	/* Define all the sections within the profile area!
		...
		Possible fields:
			...
			For Areas:
				...
				string $class		If your function is a method, set the class field with your class's name and PMX will create a new instance for it.
	...
	$profile_areas = array(
		...
		'profile_action' => array(
			...
			'areas' => array(
				...
				'exportdata' => array(
					'label' => $txt['gdrp_export'],
					'enabled' => !empty($modSettings['gdpr_enabled']) && ($context['user']['is_owner'] || $context['user']['is_admin']),
					'file' => 'Profile-Actions.php',
					'function' => 'GDPR_ExportData',
					'icon' => 'logs',
					'subsections' => array(
						'profile' => array($txt['gdrp_export_profile']),
						'posts' => array($txt['gdrp_export_posts']),
					),
					'permission' => array(
						'own' => array('profile_remove_own'),
						'any' =>  array('moderate_forum'),
					),
				),
				'deleteaccount' => array(
					'label' => $txt['deleteAccount'],
					'enabled' => ($context['user']['is_owner'] && !empty($context['user']['is_admin'])) ? false : ($context['user']['is_owner'] || $context['user']['is_admin']),
					'file' => 'Profile-Actions.php',
...
function loadCustomFields($memID, $area = 'summary')
{
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		...
		$output_html = $value;
		if($row['col_name'] == 'cust_loca' && isset($txt['cust_loca']))
		{
				 $row['field_name'] = $txt['cust_loca']['field_name'];
				 $row['field_desc'] = $txt['cust_loca']['field_desc'];
		}
