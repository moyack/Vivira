...
{
	global $sourcedir, $modSettings, $user_info, $pmxcFunc, $pmxCacheFunc;
	...
	$request = $pmxcFunc['db_query']('', '
		SELECT id_member, member_name, real_name, CASE WHEN id_group = {int:admin_group} OR FIND_IN_SET({int:admin_group}, additional_groups) != 0 THEN 1 ELSE 0 END AS is_admin
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		...
		$user_log_details[$row['id_member']] = array($row['id_member'], $row['member_name'], $row['real_name']);
	}
	...
	// remove all quotes and mentions from posts.
	foreach($log_changes as $data)
	{
		$uID = $data['extra']['member'];
		$uName = $data['extra']['name'];
		$rName = $data['extra']['real_name'];

		// replace [member=id]name[/member]
		$pmxcFunc['db_query']('', '
			UPDATE {db_prefix}messages
			SET body = REPLACE(body, {string:body_old}, {string:body_new})',
			array(
				'body_old' => '[member='. $uID .']'. $uName .'[/member]',
				'body_new' => '@Guest',
			)
		);

		// replace [member=id]realname[/member]
		$pmxcFunc['db_query']('', '
			UPDATE {db_prefix}messages
			SET body = REPLACE(body, {string:body_old}, {string:body_new})',
			array(
				'body_old' => '[member='. $uID .']'. $rName .'[/member]',
				'body_new' => '@Guest',
			)
		);
		...
		// replace [qoute author=name]
		$pmxcFunc['db_query']('', '
			UPDATE {db_prefix}messages
			SET body = REPLACE(body, {string:body_old}, {string:body_new})',
			array(
				'body_old' => '[quote author='. $uName,
				'body_new' => '[quote author=Guest',
			)
		);

		// replace [qoute author=realname]
		$pmxcFunc['db_query']('', '
			UPDATE {db_prefix}messages
			SET body = REPLACE(body, {string:body_old}, {string:body_new})',
			array(
				'body_old' => '[quote author='. $rName,
				'body_new' => '[quote author=Guest',
			)
		);

		// replace modified name
		$pmxcFunc['db_query']('', '
			UPDATE {db_prefix}messages
			SET modified_name = REPLACE(modified_name, {string:modify_old}, {string:modify_new})',
			array(
				'modify_old' => $uName,
				'modify_new' => 'Guest',
			)
	...
	$table_list_deletes = array(
		...
		'log_member_notices' => array('id_notice'),
		'log_actions' => array('id_member'),
		'log_polls' => array('id_member'),
		'log_subscribed' => array('id_member'),
		'log_banned' => array('id_member'),
		'log_online' => array('id_member'),
		'log_group_requests' => array('id_member'),
		'member_logins' => array('id_member'),
		'mentions' => array('id_member', 'id_mentioned'),
		'themes' => array('id_member'),
		'user_alerts' => array('id_member', 'id_member_started'),
		'user_alerts_prefs' => array('id_member'),
		'user_drafts' => array('id_member'),
	...
	foreach($table_list_deletes as $table => $data)
	{
		$pmxcFunc['db_query']('', '
			DELETE FROM {db_prefix}'. $table .'
			WHERE '. $data[0] .' IN ({array_int:users})'. (isset($data[1]) ? ' OR '. $data[1] .' IN ({array_int:users})' : '') .'',
			array(
			'users' => $users,
			)
		);
	...
	$pmxcFunc['db_query']('', '
		UPDATE {db_prefix}messages
		SET id_member = {int:guest_id}, poster_email = {string:blank_email}, poster_ip = {inet:default_ip}, poster_name = {string:default_name}
		WHERE id_member IN ({array_int:users})',
		array(
			'guest_id' => 0,
			'blank_email' => '',
	...
	$pmxcFunc['db_query']('', '
		...
		SET id_member_from = {int:guest_id}, from_name = {string:guest_str}
...
function registerMember(&$regOptions, $return_errors = false)
{
	...
	$regOptions['register_vars'] = array(
		...
		'gdpr_date' => strtotime($modSettings['gdpr_last_update']) + 43200,
	);
