...
function getFileVersions(&$versionOptions)
{
	...
	if ($settings['theme_id'] != 1)
		$directories += array('template_versions' => array('' => $settings['theme_dir'], 'Portal/' => $settings['theme_dir'] .'/Portal'));
...
function emailAdmins($template, $replacements = array(), $additional_recipients = array())
{
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		...
		$emaildata = loadEmailTemplate($template, $replacements, empty($row['lngfile']) ? $language : $row['lngfile']);
	...
	if (!empty($additional_recipients))
		foreach ($additional_recipients as $recipient)
		{
			...
			$emaildata = loadEmailTemplate($template, $replacements, empty($recipient['lang']) ? $language : $recipient['lang']);
