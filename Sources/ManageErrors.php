...
function ViewErrorLog()
{
	...
	for ($i = 0; $row = $pmxcFunc['db_fetch_assoc']($request); $i ++)
	{
		...
		$context['errors'][$row['id_error']] = array(
			...
			'message' => array(
				'html' => htmlspecialchars_decode($show_message),
	...
	if (isset($filter))
	{
		...
		elseif ($filter['variable'] == 'url')
			$context['filter']['value']['html'] = '\'' . strtr((substr($filter['value']['sql'], 0, 1) == '?' ? $scripturl : '') . $filter['value']['sql'], array('\_' => '_')) . '\'';
		elseif ($filter['variable'] == 'error_type')
			$context['filter']['value']['html'] = '\'' . strtr($filter['value']['sql'], array("\n" => '<br>', '&lt;br /&gt;' => '<br>', "\t" => '&nbsp;&nbsp;&nbsp;', '\_' => '_', '\\%' => '%', '\\\\' => '\\')) . '\'';
		else
			$context['filter']['value']['html'] = &$filter['value']['sql'];
		$context['filter']['value']['html'] = htmlspecialchars_decode($context['filter']['value']['html']);
	}
