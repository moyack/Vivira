...
function loadUserSettings($checkOnly = false)
{
	...
	if (!empty($modSettings['userLanguage']) && !defined('is_sheduled_task'))
	{
		...
		// Is it valid?
		if (!empty($_POST['headlangsel']) && isset($languages[strtr($_POST['headlangsel'], './\\:', '____')]))
		{
			$user_info['language'] = strtr($_POST['headlangsel'], './\\:', '____');
		...
		else
		{
			if (checkECL_Cookie())
			{
				if (!empty($_SESSION['language']) && isset($languages[strtr($_SESSION['language'], './\\:', '____')]))
					$user_info['language'] = strtr($_SESSION['language'], './\\:', '____');
				elseif ($user_info['is_guest'] && ($temp_lang = get_cookie('language')) !== null && isset($languages[strtr($temp_lang, './\\:', '____')]))
					$_SESSION['language'] = $user_info['language'] = $temp_lang;
			}
			else if (($temp_lang = get_cookie('language')) !== null && isset($languages[strtr($temp_lang, './\\:', '____')]))
				$user_info['language'] = strtr($temp_lang, './\\:', '____');
		}
	}
...
function loadLanguage($template_name, $lang = '', $fatal = true, $force_reload = false)
{
	...
	if(!defined('is_sheduled_task'))
	{
		...
		$force_reload = (!empty($lang) && $lang != $user_info['language']);
	}
