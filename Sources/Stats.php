...
function DisplayStats()
{
	...
	{
		$result = $pmxcFunc['db_query']('', '
			SELECT COUNT(id_member) AS total_members, gender
		...
		while ($row = $pmxcFunc['db_fetch_assoc']($result))
			$context['gender'][$txt['gender_type'][$row['gender']]] = $row['total_members'];

		$pmxcFunc['db_free_result']($result);
		for($i = 0; $i < 4; $i++)
		{
			if(!isset($context['gender'][$txt['gender_type'][$i]]))
				$context['gender'][$txt['gender_type'][$i]] = 0;
		}
	}
...
{
	global $context, $pmxcFunc, $user_info, $language, $txt, $modSettings;
	...
	while ($row_days = $pmxcFunc['db_fetch_assoc']($days_result))
	{
		$context['yearly'][$row_days['stats_year']]['months'][(int) $row_days['stats_month']]['days'][] = array(
			'year' => sprintf($modSettings['stats_oder'][0][0], $row_days[$modSettings['stats_oder'][0][1]]),
			'month' => sprintf($modSettings['stats_oder'][1][0], $row_days[$modSettings['stats_oder'][1][1]]),
			'day' => sprintf($modSettings['stats_oder'][2][0], $row_days[$modSettings['stats_oder'][2][1]]),
