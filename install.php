...
$GLOBALS['current_pmx_version'] = 'PortaMx-Forum 1.41';
$GLOBALS['db_script_version'] = '1-41';

$GLOBALS['required_php_version'] = '5.5';
...
function DatabaseSettings()
{
	...
	if (isset($_POST['db_user']))
	{
		...
		$_SESSION['use_innnoDB'] = isset($_POST['db_innodb']) && !empty($_POST['db_innodb']);
...
function ForumSettings()
{
	...
	$_SESSION['use_innodb'] = isset($_POST['db_innodb']) && !empty($_POST['db_innodb']);
	$incontext['continue'] = 1;
	...
	if (substr($db_type, 0, 5) == 'mysql')
	{
		...
		while ($row = $pmxcFunc['db_fetch_assoc']($get_engines))
		{
			if ($row['Support'] == 'YES' || $row['Support'] == 'DEFAULT')
				$engines[] = strtoupper($row['Engine']);
		}

		// Done with this now
		$pmxcFunc['db_free_result']($get_engines);

		// InnoDB or MyIsam .. what is the best choice? Hmm .. I prefer MyIsam ;-) .. but anyone have the choice
		$has_innodb = in_array('INNODB', $engines) && isset($_SESSION['use_innnoDB']) && !empty($_SESSION['use_innnoDB']);
...
function AdminAccount()
{
	...
	$pmxcFunc['strtolower'] = $db_character_set != 'utf8' && $txt['lang_character_set'] != 'UTF-8' ? 'strtolower' : function($string) {
		global $sourcedir;
...
<html', $txt['lang_rtl'] == true ? ' dir="rtl"' : '', '>
	<head>
		...
		<link rel="stylesheet" href="Themes/default/css/index.css?pmx11">
		<link rel="stylesheet" href="Themes/default/css/install.css?pmx11">
		', $txt['lang_rtl'] == true ? '<link rel="stylesheet" href="Themes/default/css/rtl.css?pmx11">' : '' , '
...
// Welcome them to the wonderful world!
...
function template_database_settings()
{
	...
	echo '
			<tr id="db_server_contain">
				...
				<td valign="top" class="textbox"><label for="db_innodb_input">', $txt['db_settings_innodb'], ':</label></td>
				<td>
					<input type="checkbox" name="db_innodb" id="db_innodb" value="1" class="input_check" /><br />
					<div class="smalltext block">', $txt['db_settings_innodb_info'], '</div>
				</td>
			</tr>
		</table>';
